...
TEST_CASE("binary_zero_max", "[binary]") {
    ...
    REQUIRE(ct_little_endian.get_uint16(buf) == 0);
    ct_little_endian.put_uint32(buf, 0);
    REQUIRE(ct_little_endian.get_uint32(buf) == 0);
    ct_little_endian.put_uint64(buf, 0);
    REQUIRE(ct_little_endian.get_uint64(buf) == 0);
    ct_little_endian.put_uint16(buf, UINT16_MAX);
    REQUIRE(ct_little_endian.get_uint16(buf) == UINT16_MAX);
    ct_little_endian.put_uint32(buf, UINT32_MAX);
    REQUIRE(ct_little_endian.get_uint32(buf) == UINT32_MAX);
    ct_little_endian.put_uint64(buf, UINT64_MAX);
    REQUIRE(ct_little_endian.get_uint64(buf) == UINT64_MAX);
}
...
TEST_CASE("binary_unaligned", "[binary]") {
    ...
    REQUIRE(ct_little_endian.get_uint32(buf) == 0xDEADBEEF);
    ct_big_endian.put_uint64(buf, 0x0123456789ABCDEFULL);
    REQUIRE(ct_big_endian.get_uint64(buf) == 0x0123456789ABCDEFULL);
}

TEST_CASE("binary_bswap16_batch", "[binary]") {
    uint16_t data[16] = {0x0001,0x0102,0x0203,0x0304,0x0405,0x0506,0x0607,0x0708,0x0809,0x090A,0x0A0B,0x0B0C,0x0C0D,0x0D0E,0x0E0F,0x0F10};
...
TEST_CASE("binary_bswap16_x2", "[binary]") {
    REQUIRE(ct_binary_bswap16_x2(0x11223344) == 0x22114433);
    REQUIRE(ct_binary_bswap16_x2(0x00000000) == 0x00000000);
    REQUIRE(ct_binary_bswap16_x2(0xFFFFFFFF) == 0xFFFFFFFF);
}

TEST_CASE("binary_bswap16_x4", "[binary]") {
    REQUIRE(ct_binary_bswap16_x4(0x1122334455667788ULL) == 0x2211443366558877ULL);
    REQUIRE(ct_binary_bswap16_x4(0x0000000000000000ULL) == 0x0000000000000000ULL);
}
...
TEST_CASE("binary_reverse_words32", "[binary]") {
    REQUIRE(ct_binary_reverse_words32(0x11223344) == 0x33441122);
    REQUIRE(ct_binary_reverse_words32(0x00000000) == 0x00000000);
    REQUIRE(ct_binary_reverse_words32(0xFFFFFFFF) == 0xFFFFFFFF);
    REQUIRE(ct_binary_reverse_words32(0x01020304) == 0x03040102);
}

TEST_CASE("binary_reverse_words64", "[binary]") {
    REQUIRE(ct_binary_reverse_words64(0x1122334455667788ULL) == 0x7788556633441122ULL);
    REQUIRE(ct_binary_reverse_words64(0x0000000000000000ULL) == 0x0000000000000000ULL);
    REQUIRE(ct_binary_reverse_words64(0xFFFFFFFFFFFFFFFFULL) == 0xFFFFFFFFFFFFFFFFULL);
}
...
TEST_CASE("binary_interface_and_roundtrip", "[binary]") {
    REQUIRE(ct_little_endian.endian == CT_ENDIAN_LITTLE);
    REQUIRE(ct_big_endian.endian == CT_ENDIAN_BIG);
    uint8_t buf[8] = {0};
    for (uint16_t i = 0; i < 1000; ++i) {
        ct_little_endian.put_uint16(buf, i);
        REQUIRE(ct_little_endian.get_uint16(buf) == i);
        ct_big_endian.put_uint16(buf, i);
        REQUIRE(ct_big_endian.get_uint16(buf) == i);
    }
}
