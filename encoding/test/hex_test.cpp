...
TEST_CASE("hex_decode", "[hex]") {
    ...
    REQUIRE(ct_hex_decode(NULL, 0, output, sizeof(output)) == -1);
    REQUIRE(ct_hex_decode(hex, 0, NULL, sizeof(output)) == -1);
...
TEST_CASE("hex_encode_edge", "[hex]") {
    ...
    REQUIRE(ct_hex_encode(NULL, sizeof(data), output, sizeof(output)) == -1);
    REQUIRE(ct_hex_encode(data, sizeof(data), NULL, sizeof(output)) == -1);
