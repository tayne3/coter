...
static void test_bytepool_basic_operations(void) {
	...
	for (int i = 0; i < 15; i++) {
		...
		assert_uint32_eq(ct_bytes_capacity(bytes), 100);
		assert_uint32_eq(ct_bytes_size(bytes), 0);
		assert_uint32_eq(ct_bytes_available(bytes), 100);
...
static void test_bytepool_concurrency(void) {
	...
	assert_int32_eq(available_bytes, CONCURRENCY_THREADS * 2);
...
static void test_bytepool_performance(void) {
	...
	// assert_true(time_with_pool < time_without_pool);
}
