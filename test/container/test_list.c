...
static inline void test_list_basic(void) {
	...
	ctunit_assert_pointer(ct_list_entry(node1.list, my_struct_t, list), &node1);
	ctunit_assert_pointer(ct_list_entry(node2.list, my_struct_t, list), &node2);
	ctunit_assert_pointer(ct_list_entry(node3.list, my_struct_t, list), &node3);

	ct_list_append(head, node1.list);
	ctunit_assert_false(ct_list_isempty(head));
	ctunit_assert_uint32(ct_list_size(head), 1, CTUnit_Equal);
	...
	ctunit_assert_false(ct_list_isempty(head));
	ctunit_assert_uint32(ct_list_size(head), 2, CTUnit_Equal);
	ctunit_assert_true(ct_list_first(head) == node1.list);
	ctunit_assert_true(ct_list_last(head) == node2.list);

	ct_list_append(head, node3.list);
	ctunit_assert_false(ct_list_isempty(head));
	ctunit_assert_uint32(ct_list_size(head), 3, CTUnit_Equal);
	ctunit_assert_true(ct_list_first(head) == node1.list);
...
static inline void test_list_operations(void) {
	...
	ctunit_assert_true(ct_list_isempty(head));
	ctunit_assert_uint32(ct_list_size(head), 0, CTUnit_Equal);
	ctunit_assert_pointer(ct_list_first(head), head);
	...
	ctunit_assert_uint32(ct_list_size(head), 3, CTUnit_Equal);
	ctunit_assert_pointer(ct_list_first(head), node2);
	ctunit_assert_pointer(ct_list_last(head), node3);
	...
	ctunit_assert_false(ct_list_isempty(head));
	ctunit_assert_uint32(ct_list_size(head), 4, CTUnit_Equal);

	ctunit_assert_pointer(ct_list_first(head), node3);
	ctunit_assert_pointer(ct_list_last(head), node4);
...
static inline void test_list_splice(void) {
	...
	ctunit_assert_uint32(ct_list_size(head1), 4, CTUnit_Equal);
	ctunit_assert_uint32(ct_list_size(head2), 0, CTUnit_Equal);
	ctunit_assert_pointer(ct_list_first(head1), node3);
	ctunit_assert_pointer(ct_list_last(head1), node2);

	count = 0;
	ct_list_foreach (pos, head1) {
		switch (count) {
			case 0: ctunit_assert_pointer(pos, node3); break;
			case 1: ctunit_assert_pointer(pos, node4); break;
			case 2: ctunit_assert_pointer(pos, node1); break;
			case 3: ctunit_assert_pointer(pos, node2); break;
		}
		count++;
	}
	ctunit_assert_int(count, 4, CTUnit_Equal);
	...
	ctunit_assert_uint32(ct_list_size(head1), 6, CTUnit_Equal);
	ctunit_assert_uint32(ct_list_size(head3), 0, CTUnit_Equal);
	ctunit_assert_pointer(ct_list_first(head1), node3);
	ctunit_assert_pointer(ct_list_last(head1), node6);
	...
	ct_list_foreach (pos, head1) {
		switch (count) {
			...
			case 2: ctunit_assert_pointer(pos, node1); break;
			case 3: ctunit_assert_pointer(pos, node2); break;
			case 4: ctunit_assert_pointer(pos, node5); break;
			case 5: ctunit_assert_pointer(pos, node6); break;
		}
		count++;
	}
	ctunit_assert_int(count, 6, CTUnit_Equal);
	...
	ctunit_assert_uint32(ct_list_size(head1), 0, CTUnit_Equal);
...
static inline void test_list_foreach_entry(void) {
	...
	ct_list_append(head, node1.list);
	ctunit_assert_pointer(ct_list_first(head), node1.list);
	ctunit_assert_pointer(ct_list_last(head), node1.list);
