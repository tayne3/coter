...
static inline int test_stack_init(void) {
...

	{
		...
		for (i = 1; i <= max; i++) {
			...
			ctunit_assert_int(ct_stack_max(&stack), i, CTUnit_Equal);
			ctunit_assert_int(ct_stack_size(&stack), 0, CTUnit_Equal);
...
static inline int test_stack_push(void) {
	...
	ctunit_assert_int(ct_stack_max(&stack), max, CTUnit_Equal);
	ctunit_assert_int(ct_stack_size(&stack), 0, CTUnit_Equal);
	ctunit_assert_true(ct_stack_isempty(&stack));
	ctunit_assert_true(!ct_stack_isfull(&stack));
...
static inline int test_stack_top(void) {
...

	{
		...
		for (i = 1; i <= max; i++) {
			...
			ctunit_assert_int(ct_any_value_int(item), max - i + 1, CTUnit_Equal);
			ctunit_assert_int(ct_any_value_int(item_prev), max - i + 1, CTUnit_Equal);
			ctunit_assert_int(ct_stack_max(&stack), max, CTUnit_Equal);
			ctunit_assert_int(ct_stack_size(&stack), max - i, CTUnit_Equal);
			ctunit_assert_true(!ct_stack_isfull(&stack));
			if (i == max) {
				ctunit_assert_true(ct_stack_isempty(&stack));
			} else {
				...
				ctunit_assert_int(ct_any_value_int(item_next), max - i, CTUnit_Equal);
			}
		}
	}
