...
static void test_bytepool_basic_operations(void) {
	...
	for (int i = 0; i < 15; i++) {
		...
		ctunit_assert_uint32(ct_bytes_capacity(bytes), 100, CTUnit_Equal);
		ctunit_assert_uint32(ct_bytes_size(bytes), 0, CTUnit_Equal);
		ctunit_assert_uint32(ct_bytes_available(bytes), 100, CTUnit_Equal);
	...
	ctunit_assert_int(count, 20, CTUnit_Equal);
...
static void test_bytepool_edge_cases(void) {
	...
	ctunit_assert_uint32(small_bytes->cap, MIN_BUFFER_SIZE, CTUnit_Equal);
	...
	ctunit_assert_uint32(large_bytes->cap, MAX_BUFFER_SIZE, CTUnit_Equal);
...
static void test_bytepool_concurrency(void) {
	...
	ctunit_assert_int(available_bytes, CONCURRENCY_THREADS * 2, CTUnit_Equal);
