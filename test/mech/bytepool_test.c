...
static void test_bytepool_basic_operations(void) {
	...
	cunit_assert_not_null(pool);

	// 测试获取和放回字节
	for (int i = 0; i < 15; i++) {
		ct_bytes_t* bytes = ct_bytepool_get(pool);
		cunit_assert_not_null(bytes);
		cunit_assert_uint32_equal(ct_bytes_capacity(bytes), 100);
		cunit_assert_uint32_equal(ct_bytes_size(bytes), 0);
		cunit_assert_uint32_equal(ct_bytes_available(bytes), 100);
	...
	cunit_assert_int32_equal(count, 20);
...
static void test_bytepool_edge_cases(void) {
	...
	cunit_assert_not_null(small_pool);

	ct_bytes_t* small_bytes = ct_bytepool_get(small_pool);
	cunit_assert_not_null(small_bytes);
	cunit_assert_uint32_equal(small_bytes->cap, MIN_BUFFER_SIZE);
	...
	cunit_assert_not_null(large_pool);

	ct_bytes_t* large_bytes = ct_bytepool_get(large_pool);
	cunit_assert_not_null(large_bytes);
	cunit_assert_uint32_equal(large_bytes->cap, MAX_BUFFER_SIZE);
...
static void test_bytepool_concurrency(void) {
	...
	for (int i = 0; i < CONCURRENCY_THREADS; i++) {
		...
		cunit_assert_int32_equal(result, 0);
	}
	...
	cunit_assert_int32_equal(available_bytes, CONCURRENCY_THREADS * 2);
...
static void test_bytepool_performance(void) {
	...
	cunit_println("Performance comparison: With byte pool %" PRIu64 "ms, Without byte pool %" PRIu64 "ms\n",
				 time_with_pool, time_without_pool);
	// cunit_assert_true(time_with_pool < time_without_pool);
}
