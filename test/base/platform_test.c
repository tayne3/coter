...
static inline void test_offset_of(void) {
	...
	cunit_assert_uint32_equal(offset_a, 0, "Case 1: OFFSET_OF(struct TestStruct, a) = %zu\n", offset_a);

	// 测试案例2：对齐后的成员偏移量
	size_t offset_b = OFFSET_OF(struct TestStruct, b);
	cunit_assert_uint32_equal(offset_b, alignment, "Case 2: OFFSET_OF(struct TestStruct, b) = %zu\n", offset_b);

	// 测试案例3：紧跟大型成员后的成员偏移量
	size_t offset_c = OFFSET_OF(struct TestStruct, c);
	cunit_assert_uint32_equal(offset_c, offset_b + sizeof(double), "Case 3: OFFSET_OF(struct TestStruct, c) = %zu\n",
							   offset_c);

	// 测试案例4：数组成员的偏移量
	size_t offset_d = OFFSET_OF(struct TestStruct, d);
	cunit_assert_uint32_equal(offset_d, offset_c + sizeof(int), "Case 4: OFFSET_OF(struct TestStruct, d) = %zu\n",
							   offset_d);

	// 测试案例5：最后一个成员的偏移量
	size_t offset_e = OFFSET_OF(struct TestStruct, e);
	cunit_assert_uint32_equal(offset_e, (offset_d + sizeof(char[4]) + alignment - 1) & ~(alignment - 1),
	...
	cunit_assert_uint32_equal(struct_size, expected_size, "Case 6: sizeof(struct TestStruct) = %zu\n", struct_size);

	// 测试案例7：数组元素的偏移量
	size_t offset_d2 = OFFSET_OF(struct TestStruct, d[2]);
	cunit_assert_uint32_equal(offset_d2, offset_d + 2, "Case 7: OFFSET_OF(struct TestStruct, d[2]) = %zu\n",
							   offset_d2);
}
...
static inline void test_container_of(void) {
	...
	cunit_assert_pointer_equal(container_ptr1, &test_instance1, "Case 1: original = %p, container = %p\n",
	...
	cunit_assert_pointer_equal(container_ptr2, &test_instance1, "Case 2: original = %p, container = %p\n",
	...
	cunit_assert_pointer_equal(container_ptr3, &test_instance1, "Case 3: original = %p, container = %p\n",
								(void *)&test_instance1, (void *)container_ptr3);

	// 测试案例4：验证成员值的正确性
	cunit_assert_double_equal(container_ptr1->a, 1.0, "Case 4: a = %f\n", container_ptr1->a);
	cunit_assert_int32_equal(container_ptr1->b, 2, "Case 4: b = %d\n", container_ptr1->b);
	cunit_assert_char(container_ptr1->c[0], 'a', "Case 4: c[0] = %c\n", container_ptr1->c[0]);
	cunit_assert_float_equal(container_ptr1->d, 3.14f, "Case 4: d = %f\n", container_ptr1->d);
	...
	cunit_assert_pointer_equal(container_ptr5, &test_array[1], "Case 5: original = %p, container = %p\n",
