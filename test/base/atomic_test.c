...
static inline void test_atomic_add_sub(void) {
	...
	ctunit_assert_int64(CT_ATOMIC_LOAD(&test_value), 0, CTUnit_Equal);

	CT_ATOMIC_ADD(&test_value, 10);
	ctunit_assert_int64(CT_ATOMIC_LOAD(&test_value), 10, CTUnit_Equal);

	CT_ATOMIC_SUB(&test_value, 5);
	ctunit_assert_int64(CT_ATOMIC_LOAD(&test_value), 5, CTUnit_Equal);
...
static inline void test_atomic_load_store(void) {
	...
	ctunit_assert_int64(CT_ATOMIC_LOAD(&test_value), 42, CTUnit_Equal);

	CT_ATOMIC_STORE(&test_value, 100);
	ctunit_assert_int64(CT_ATOMIC_LOAD(&test_value), 100, CTUnit_Equal);

	// 测试边界值
	CT_ATOMIC_STORE(&test_value, LONG_MAX);
	ctunit_assert_int64(CT_ATOMIC_LOAD(&test_value), LONG_MAX, CTUnit_Equal);

	CT_ATOMIC_STORE(&test_value, LONG_MIN);
	ctunit_assert_int64(CT_ATOMIC_LOAD(&test_value), LONG_MIN, CTUnit_Equal);
}
...
static inline void test_atomic_operations_multi_threaded(void) {
	...
	ctunit_assert_int64(CT_ATOMIC_LOAD(&test_value), NUM_THREADS, CTUnit_Equal);
}
