...
struct Recurse {
	Recurse() { u = bit_cast<uint32_t>(0.f); }
	uint32_t u;
};

struct RecurseInit {
	uint32_t u = bit_cast<uint32_t>(0.f);
};
...
TEST_CASE("bit_cast: float to uint32_t", "[bit_cast][basic]") {
	REQUIRE(bit_cast<uint32_t>(0.0f) == 0x00000000);
	REQUIRE(bit_cast<uint32_t>(-0.0f) == 0x80000000);
	REQUIRE(bit_cast<uint32_t>(2.0f) == 0x40000000);
}

TEST_CASE("bit_cast: uint32_t to float", "[bit_cast][basic]") {
	REQUIRE(bit_cast<float>(0x00000000u) == 0.0f);
	REQUIRE(bit_cast<float>(0x40000000u) == 2.0f);
}

TEST_CASE("bit_cast: const qualifiers", "[bit_cast][basic]") {
	const float cf = 2.0f;
	REQUIRE(bit_cast<uint32_t>(cf) == 0x40000000);
}
...
TEST_CASE("bit_cast: Float struct (bit fields)", "[bit_cast][struct]") {
	...
	REQUIRE(bit_cast<float>(Float{0, 0, 0}) == 0.0f);
	// Float{mantissa=0, exponent=0, sign=1} -> -0.0f
	REQUIRE(std::signbit(bit_cast<float>(Float{0, 0, 1})));
	// Float{mantissa=0, exponent=0x80, sign=0} -> 2.0f
	REQUIRE(bit_cast<float>(Float{0, 0x80, 0}) == 2.0f);
}

TEST_CASE("bit_cast: Padded struct", "[bit_cast][struct]") {
	Padded p = bit_cast<Padded>(2.0f);
...
#endif
	REQUIRE(bit_cast<uint32_t>(arr) == 0xDEADBEEF);
}

#ifdef __GNUC__
TEST_CASE("bit_cast: SIMD vector type V4x8", "[bit_cast][array]") {
	V4x8 v = bit_cast<V4x8>(0xDEADBEEFu);
#if INDDIAG_ENDIAN_IS_BIG
	REQUIRE(v[0] == 0xDE);
	REQUIRE(v[1] == 0xAD);
	REQUIRE(v[2] == 0xBE);
...
TEST_CASE("float64_from_bits: uint32_t overload promotes", "[float_bits][float64]") {
	...
	REQUIRE(d == bit_cast<double>(0x000000003FF00000ULL));
}
