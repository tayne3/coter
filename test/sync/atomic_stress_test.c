...
static Node
	g_nodes[NUM_THREADS * ITEMS_PER_THREAD];  // Pre-allocated nodes to avoid malloc/free overhead and ABA from reuse
...
static void test_atomic_ptr_stress(void) {
	...
	for (long i = 0; i < NUM_THREADS; i++) {
		pthread_create(&threads[i], NULL, thread_push_routine, (void*)(intptr_t)i);
	}
	for (int i = 0; i < NUM_THREADS; i++) {
		pthread_join(threads[i], NULL);
	}
	...
	// 2. Concurrent Pop
	for (long i = 0; i < NUM_THREADS; i++) {
		pthread_create(&threads[i], NULL, thread_pop_routine, (void*)(intptr_t)i);
	}
