 ...
 * @param timer 定时器指针
 */
COTER_API void ct_timer_init(ct_timer_t* timer);
 ...
 * @param arg 回调参数
 */
COTER_API int ct_timer_start(ct_timer_t* timer, ct_time64_t timeout_ms, ct_timer_callback_t cb, void* arg);
 ...
 * @param timeout_ms 超时时间 (ms)
 */
COTER_API int ct_timer_reset(ct_timer_t* timer, ct_time64_t timeout_ms);

/**
 * @brief 停止定时器
 * @param timer 定时器指针
 */
COTER_API int ct_timer_stop(ct_timer_t* timer);
 ...
 */
COTER_API void ct_timer_mgr_init(ct_time64_t (*gettime_cb)(void));

/**
 * @brief 运行定时器管理器
 * @note 阻塞运行, 使用 ct_timer_mgr_close 停止
 */
COTER_API void ct_timer_mgr_run(void);
