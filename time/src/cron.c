...
static inline void ct_cron_time_correction_callback(void *arg) {
	...
	for (size_t i = 0; i < size; ++i) { array[i] = CT_CRON_HEAP_POP(); }
...
bool ct_cron_mgr_schedule(ct_time_t now) {
	CT_CRON_MGR_LOCK();
	// 系统时间被修改, 重启所有cron任务
	if (g_cron_mgr->current_time > now || g_cron_mgr->current_time + CT_CRON_TIME_TOLERANCE_MS < now) { g_cron_mgr->time_correction_pending++; }
	g_cron_mgr->current_time = now;
