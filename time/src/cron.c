...
static int  cron__start(ct_cron_t* cron, ct_time_t now, int minute, int hour, int day, int week, int month, ct_cron_callback_t cb, void* arg);
static int  cron__stop(ct_cron_t* cron);
static int  cron__reschedule(ct_cron_t* cron, ct_time_t now);
...
int ct_cron_start(ct_cron_t* cron, int minute, int hour, int day, int week, int month, ct_cron_callback_t callback, void* arg) {
...
void ct_cron_mgr_run(void) {
	...
	while (!ct_atomic_bool_load(&g_mgr.is_shutdown)) {
		...
		cron__reschedule(cron, g_mgr.now);
...
static int cron__start(ct_cron_t* cron, ct_time_t now, int minute, int hour, int day, int week, int month, ct_cron_callback_t cb, void* arg) {
	...
	return cron__reschedule(cron, now);
}
...
static bool cron__handle_time_jump() {
	...
	while (1) {
		...
		if (!cron) { break; }
		if (cron->is_active) { cron__reschedule(cron, g_mgr.now); }
	}
	return true;
}
