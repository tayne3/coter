...
static const char* s_months[] = {"January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"};
...
ct_datetime_t* ct_datetime_past(ct_datetime_t* dt, int days) {
	if (!dt) { return NULL; }
	if (days < 0) { return ct_datetime_future(dt, -days); }
	int sub = days;
	while (sub) {
		if (dt->day > sub) {
			dt->day -= sub;
...
ct_datetime_t* ct_datetime_future(ct_datetime_t* dt, int days) {
	if (!dt) { return NULL; }
	if (days < 0) { return ct_datetime_past(dt, -days); }
	int sub = days;
	int mdays;
	while (sub) {
...
char* ct_datetime_gmtime_fmt(time_t t, char* buf) {
	...
	sprintf(buf, CT_GMTIME_FMT, s_weekdays[tm->tm_wday], tm->tm_mday, s_months[tm->tm_mon], tm->tm_year + 1900, tm->tm_hour, tm->tm_min, tm->tm_sec);
	return buf;
}
...
const char* ct_datetime_month_itoa(int month) {
	if (month <= 0 || month > 12) { return STR_NULL; }
...
const char* ct_datetime_weekday_itoa(int weekday) {
	if (weekday < 0 || weekday > 7) { return STR_NULL; }
