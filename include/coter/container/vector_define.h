...
#define CT_VECTOR_DEFINE(TYPE, NAME)                                                                                            \
	...
	bool ct_vector_##NAME##_resize(ct_vector_##NAME##_t* self, size_t new_size) {                                               \
		...
		if (new_size > self->_cap) {                                                                                            \
			if (!ct_vector_##NAME##_reserve(self, new_size)) { return false; }                                                  \
		}                                                                                                                       \
		if (new_size > self->_size) { memset(&self->_all[self->_size], 0, (new_size - self->_size) * sizeof(TYPE)); }           \
		self->_size = new_size;                                                                                                 \
		return true;                                                                                                            \
	}                                                                                                                           \
	bool ct_vector_##NAME##_reserve(ct_vector_##NAME##_t* self, size_t capacity) {                                              \
		assert(self);                                                                                                           \
		if (self->_cap >= capacity) { return true; }                                                                            \
		size_t new_cap = self->_cap == 0 ? 1 : self->_cap;                                                                      \
		while (new_cap < capacity) { new_cap *= 2; }                                                                            \
		if (sizeof(TYPE) * new_cap > CT_VECTOR_MEMORY_MAX) { return false; /* out of memory */ }                                \
		TYPE* new_all = (TYPE*)realloc(self->_all, sizeof(TYPE) * new_cap);                                                     \
		if (!new_all) { return false; /* realloc failed */ }                                                                    \
		self->_all = new_all;                                                                                                   \
		self->_cap = new_cap;                                                                                                   \
		return true;                                                                                                            \
	}                                                                                                                           \
	bool ct_vector_##NAME##_shrink(ct_vector_##NAME##_t* self) {                                                                \
		assert(self);                                                                                                           \
		if (self->_size == self->_cap) { return true; }                                                                         \
		if (self->_size == 0) {                                                                                                 \
			free(self->_all);                                                                                                   \
			self->_all = NULL;                                                                                                  \
			self->_cap = 0;                                                                                                     \
	...
	bool ct_vector_##NAME##_insert(ct_vector_##NAME##_t* self, size_t idx, TYPE data) {                                         \
		assert(self);                                                                                                           \
		if (idx > self->_size) { return false; }                                                                                \
		if (!ct_vector_##NAME##_reserve(self, self->_size + 1)) { return false; }                                               \
		if (idx < self->_size) { memmove(&self->_all[idx + 1], &self->_all[idx], (self->_size - idx) * sizeof(TYPE)); }         \
		self->_all[idx] = data;                                                                                                 \
		++self->_size;                                                                                                          \
		return true;                                                                                                            \
	}                                                                                                                           \
	...
	bool ct_vector_##NAME##_erase(ct_vector_##NAME##_t* self, size_t idx) {                                                     \
		assert(self);                                                                                                           \
		if (idx >= self->_size) { return false; }                                                                               \
		if (idx < self->_size - 1) { memmove(&self->_all[idx], &self->_all[idx + 1], (self->_size - idx - 1) * sizeof(TYPE)); } \
		--self->_size;                                                                                                          \
		return true;                                                                                                            \
	}                                                                                                                           \
	bool ct_vector_##NAME##_pop(ct_vector_##NAME##_t* self) {                                                                   \
