...
static const ct_opt_long_t kLongopts[] = {
	{"amend", 'a', CT_OPT_NONE, NULL, NULL},     {"brief", 'b', CT_OPT_NONE, NULL, NULL}, {"color", 'c', CT_OPT_OPTIONAL, NULL, NULL},
	{"delay", 'd', CT_OPT_REQUIRED, NULL, NULL}, {"erase", 'e', CT_OPT_NONE, NULL, NULL}, {"file", 'f', CT_OPT_REQUIRED, NULL, NULL},
...
TEST_CASE("short: no arguments", "[short]") {
	Argv av{};
	auto o = av.to_opts();
	REQUIRE(ct_opt_parse(&o, "abc") == -1);
}

TEST_CASE("short: single flag", "[short]") {
	Argv av{"-a"};
	auto o = av.to_opts();
	REQUIRE(ct_opt_parse(&o, "abc") == 'a');
	REQUIRE(ct_opt_parse(&o, "abc") == -1);
}
...
TEST_CASE("short: multiple flags", "[short]") {
	...
	auto o = av.to_opts();
	REQUIRE(ct_opt_parse(&o, "abc") == 'a');
	REQUIRE(ct_opt_parse(&o, "abc") == 'b');
	REQUIRE(ct_opt_parse(&o, "abc") == 'c');
	REQUIRE(ct_opt_parse(&o, "abc") == -1);
}
...
TEST_CASE("short: unknown option returns '?'", "[short][error]") {
	Argv av{"-z"};
	auto o = av.to_opts();
	REQUIRE(ct_opt_parse(&o, "abc") == '?');
	REQUIRE(std::string(o.errmsg).find(CT_OPT_MSG_INVALID) == 0);
}

TEST_CASE("short: missing required argument returns '?'", "[short][error]") {
	Argv av{"-c"};
	auto o = av.to_opts();
	REQUIRE(ct_opt_parse(&o, "c:") == '?');
	REQUIRE(std::string(o.errmsg).find(CT_OPT_MSG_MISSING) == 0);
}
...
TEST_CASE("long: single flag --amend", "[long]") {
	...
	auto o  = av.to_opts();
	int  li = -1;
	REQUIRE(ct_opt_long(&o, kLongopts, &li) == 'a');
	REQUIRE(li == 0);
}
...
TEST_CASE("long: multiple flags", "[long]") {
	Argv av{"--amend", "--brief"};
	auto o = av.to_opts();
	REQUIRE(ct_opt_long(&o, kLongopts, nullptr) == 'a');
	REQUIRE(ct_opt_long(&o, kLongopts, nullptr) == 'b');
	REQUIRE(ct_opt_long(&o, kLongopts, nullptr) == -1);
}

TEST_CASE("long: required argument — separate token", "[long]") {
	Argv av{"--delay", "500"};
	auto o = av.to_opts();
	REQUIRE(ct_opt_long(&o, kLongopts, nullptr) == 'd');
...
TEST_CASE("long: required argument — inline with '='", "[long]") {
	Argv av{"--color=red"};
	auto o = av.to_opts();
	REQUIRE(ct_opt_long(&o, kLongopts, nullptr) == 'c');
	REQUIRE(std::string(o.optarg) == "red");
}
...
TEST_CASE("long: required argument missing returns '?'", "[long][error]") {
	Argv av{"--delay"};
	auto o = av.to_opts();
	REQUIRE(ct_opt_long(&o, kLongopts, nullptr) == '?');
	REQUIRE(std::string(o.errmsg).find(CT_OPT_MSG_MISSING) == 0);
}
...
TEST_CASE("long: TOOMANY when flag given an argument", "[long][error]") {
	...
	auto o = av.to_opts();
	REQUIRE(ct_opt_long(&o, kLongopts, nullptr) == '?');
	REQUIRE(std::string(o.errmsg).find(CT_OPT_MSG_TOOMANY) == 0);
}

TEST_CASE("long: long-only option (shortname > 127)", "[long]") {
	static const ct_opt_long_t lo[] = {
	...
	REQUIRE(ct_opt_long(&o, lo, &li) == 256);
	REQUIRE(li == 0);

	REQUIRE(ct_opt_long(&o, lo, &li) == 257);
	REQUIRE(li == 1);
...
TEST_CASE("long: mix of short and long options", "[long]") {
	/* Note: -c uses CT_OPT_OPTIONAL in kLongopts, so "-c green" will NOT
	 ...
	 * CT_OPT_REQUIRED for separate-token style. Here we use --color=green. */
	...
	int c;
	while ((c = ct_opt_long(&o, kLongopts, nullptr)) != -1) {
		switch (c) {
...
TEST_CASE("arg: basic positional collection", "[arg]") {
	Argv av{"-a", "foo", "bar"};
	auto o = av.to_opts();
	ct_opt_long(&o, kLongopts, nullptr); /* consume -a */
	ct_opt_long(&o, kLongopts, nullptr); /* returns -1 */
	auto args     = unconsumed_args(&o);
	auto expected = std::vector<std::string>{"foo", "bar"};
	REQUIRE(args == expected);
}
...
TEST_CASE("edge: color inline with combined short opts", "[edge]") {
	...
	static const ct_opt_long_t lo[] = {
		...
		{"color", 'c', CT_OPT_REQUIRED, NULL, NULL},
		{nullptr, 0, CT_OPT_NONE, NULL, NULL},
	};
	...
	std::string color;
	int         c;
	while ((c = ct_opt_long(&o, lo, nullptr)) != -1) {
		switch (c) {
			case 'a': a = true; break;
			case 'b': b = true; break;
			case 'c': color = o.optarg ? o.optarg : ""; break;
...
TEST_CASE("errmsg: INVALID contains option name", "[errmsg]") {
	Argv av{"--unknown"};
	auto o = av.to_opts();
	ct_opt_long(&o, kLongopts, nullptr);
	std::string msg(o.errmsg);
	REQUIRE(msg.find(CT_OPT_MSG_INVALID) == 0);
...
TEST_CASE("errmsg: MISSING contains option name", "[errmsg]") {
	...
	ct_opt_long(&o, kLongopts, nullptr);
	std::string msg(o.errmsg);
	REQUIRE(msg.find(CT_OPT_MSG_MISSING) == 0);
...
TEST_CASE("errmsg: TOOMANY contains option name", "[errmsg]") {
	...
	ct_opt_long(&o, kLongopts, nullptr);
	std::string msg(o.errmsg);
	REQUIRE(msg.find(CT_OPT_MSG_TOOMANY) == 0);
...
static Config run_long(char** argv_raw, const ct_opt_long_t* lo = kLongopts) {
	Config   cfg;
	ct_opt_t o;
...
TEST_CASE("regression: -- foobar", "[regression]") {
	...
	while (ct_opt_long(&o, kLongopts, nullptr) != -1) {}
	auto args = unconsumed_args(&o);
...
TEST_CASE("regression: -abcblue -d10 foobar", "[regression]") {
	...
	static const ct_opt_long_t lo[] = {
		{"amend", 'a', CT_OPT_NONE, NULL, NULL},     {"brief", 'b', CT_OPT_NONE, NULL, NULL}, {"color", 'c', CT_OPT_REQUIRED, NULL, NULL},
		{"delay", 'd', CT_OPT_REQUIRED, NULL, NULL}, {"erase", 'e', CT_OPT_NONE, NULL, NULL}, {nullptr, 0, CT_OPT_NONE, NULL, NULL},
	};
...
TEST_CASE("regression: --delay (missing arg) gives MISSING error", "[regression]") {
	Argv av{"--delay"};
	auto cfg = run_long(av.data());
	REQUIRE(std::string(cfg.err).find(CT_OPT_MSG_MISSING) == 0);
}
...
TEST_CASE("regression: --foo bar leaves foo and bar as positionals", "[regression]") {
	...
	REQUIRE(std::string(err).find(CT_OPT_MSG_INVALID) == 0);
}

TEST_CASE("regression: -x leaves -x as positional", "[regression]") {
	Argv av{"-x"};
	auto cfg = run_long(av.data());
	REQUIRE(cfg.err.find(CT_OPT_MSG_INVALID) == 0);
}
