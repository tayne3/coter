...
static inline int ct_opt__is_dashdash(const char* arg) {
	return arg && arg[0] == '-' && arg[1] == '-' && arg[2] == '\0';
}

static inline int ct_opt__is_short(const char* arg) {
	return arg && arg[0] == '-' && arg[1] != '-' && arg[1] != '\0';
}

static inline int ct_opt__is_long(const char* arg) {
	return arg && arg[0] == '-' && arg[1] == '-' && arg[2] != '\0';
}

static inline int ct_opt__is_end(const ct_opt_long_t* opt) {
...
static int ct_opt__parse_long(ct_opt_t* options, const ct_opt_long_t* longopts, int* longindex) {
	...
	for (int i = 0; !ct_opt__is_end(&longopts[i]); ++i) {
		...
		if (longopts[i].argtype == CT_OPT_NONE && val != NULL) { return ct_opt__error(options, CT_OPT_MSG_TOOMANY, name); }
...
static inline ct_opt_help_config_t ct_opt__resolve_config(const ct_opt_help_config_t* cfg) {
