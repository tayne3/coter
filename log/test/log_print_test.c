...
static inline void test_print_performance_comparison(void) {
	...
	for (int i = 0; i < TEST_THREADS; ++i) { assert_int32_eq(pthread_create(&threads[i], NULL, thread_print_without_log, NULL), 0); }
	for (int i = 0; i < TEST_THREADS; ++i) { assert_int32_eq(pthread_join(threads[i], NULL), 0); }
	end                        = ct_getuptime_ms();
	const int time_without_log = (int)(end - start);

	start = ct_getuptime_ms();
	for (int i = 0; i < TEST_THREADS; ++i) { assert_int32_eq(pthread_create(&threads[i], NULL, thread_print_with_basic_log, NULL), 0); }
	for (int i = 0; i < TEST_THREADS; ++i) { assert_int32_eq(pthread_join(threads[i], NULL), 0); }
	end                           = ct_getuptime_ms();
	...
	// 新增 brief log 测试
	start = ct_getuptime_ms();
	for (int i = 0; i < TEST_THREADS; ++i) { assert_int32_eq(pthread_create(&threads[i], NULL, thread_print_with_brief_log, NULL), 0); }
	for (int i = 0; i < TEST_THREADS; ++i) { assert_int32_eq(pthread_join(threads[i], NULL), 0); }
	end                           = ct_getuptime_ms();
	...
	// 新增 detail log 测试
	start = ct_getuptime_ms();
	for (int i = 0; i < TEST_THREADS; ++i) { assert_int32_eq(pthread_create(&threads[i], NULL, thread_print_with_detail_log, NULL), 0); }
	for (int i = 0; i < TEST_THREADS; ++i) { assert_int32_eq(pthread_join(threads[i], NULL), 0); }
	...
	cunit_println("Execution time: without log %d ms, with basic log %d ms, with brief log %d ms, with detail log %d ms\n", time_without_log,
				  time_with_basic_log, time_with_brief_log, time_with_detail_log);
