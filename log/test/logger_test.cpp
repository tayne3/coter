...
TEST_CASE("log_default_logger_zero_init") {
...
TEST_CASE("log_default_logger_is_sealed_after_first_use") {
...
TEST_CASE("log_logger_object_api") {
    ct_logger_t logger;
    ct_logger_init(&logger);
...
TEST_CASE("log_handler_belongs_to_one_logger") {
    ct_logger_t first;
    ct_logger_t second;
...
TEST_CASE("log_unstarted_logger_does_not_write") {
    callback_state state;
    ct_logger_t    logger;
    ct_logger_init(&logger);
...
TEST_CASE("log_invalid_level_is_rejected") {
    callback_state state;
    ct_logger_t    logger;
    ct_logger_init(&logger);
...
TEST_CASE("log_logger_close_flushes_handlers") {
    flush_state state;
    ct_logger_t logger;
    ct_logger_init(&logger);
...
TEST_CASE("log_logger_add_handler_after_start_fails") {
    ct_logger_t logger;
    ct_logger_init(&logger);
...
TEST_CASE("log_logger_level_is_atomic") {
    ct_logger_t logger;
    ct_logger_init(&logger);
    REQUIRE(ct_logger_start(&logger) == 0);
