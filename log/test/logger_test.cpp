...
TEST_CASE("log_logger_object_api", "[log]") {
    ...
    ct_log_handler_t* default_handler = ct_log_base_handler_create(&config);
    REQUIRE(ct_logger_add_handler(ct_logger_default(), default_handler) == -1);
    destroy_handler(default_handler);

    ct_log_handler_t* late_handler = ct_log_base_handler_create(&config);
...
TEST_CASE("log_handler_belongs_to_one_logger", "[log]") {
    ...
    ct_logger_close(&first);
    ct_logger_close(&second);
}
...
TEST_CASE("log_logger_add_handler_after_start_fails", "[log]") {
    ...
    ct_log_base_handler_config_t first_config;
    ct_log_base_handler_config_default(&first_config);
    first_config.routine  = collect_log;
    first_config.userdata = &first;
    REQUIRE(ct_logger_add_handler(&logger, ct_log_base_handler_create(&first_config)) == 0);
    REQUIRE(ct_logger_start(&logger) == 0);
    ...
    ct_log_base_handler_config_t second_config;
    ct_log_base_handler_config_default(&second_config);
    second_config.routine  = collect_log;
    second_config.userdata = &second;

    ct_log_handler_t* second_handler = ct_log_base_handler_create(&second_config);
