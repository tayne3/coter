...
static void test_write_performance_comparison(void) {
    ...
    for (int i = 0; i < TEST_THREADS; ++i) {
        REQUIRE(ct_thread_create(&threads[i], nullptr, thread_write_without_log, (void*)(uintptr_t)i) == 0);
    }
    ...
    start = ct_getuptime_ms();
    for (int i = 0; i < TEST_THREADS; ++i) {
        REQUIRE(ct_thread_create(&threads[i], nullptr, thread_write_with_log, (void*)(uintptr_t)i) == 0);
    }
    for (int i = 0; i < TEST_THREADS; ++i) { REQUIRE(ct_thread_join(threads[i], nullptr) == 0); }
    end                     = ct_getuptime_ms();
