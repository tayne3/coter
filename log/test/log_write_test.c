...
static inline void test_write_performance_comparison(void) {
	...
	assert_true(ct_access("test_log_out", 0) == 0);
	...
	for (int i = 0; i < TEST_THREADS; ++i) { assert_int32_eq(pthread_create(&threads[i], NULL, thread_write_without_log, (void*)(uintptr_t)i), 0); }
	for (int i = 0; i < TEST_THREADS; ++i) { assert_int32_eq(pthread_join(threads[i], NULL), 0); }
	end                        = ct_getuptime_ms();
	const int time_without_log = (int)(end - start);

	start = ct_getuptime_ms();
	for (int i = 0; i < TEST_THREADS; ++i) { assert_int32_eq(pthread_create(&threads[i], NULL, thread_write_with_log, (void*)(uintptr_t)i), 0); }
	for (int i = 0; i < TEST_THREADS; ++i) { assert_int32_eq(pthread_join(threads[i], NULL), 0); }
	end                     = ct_getuptime_ms();
