...
void ct_log_schedule(ct_time64_t tick) {
	...
	for (size_t i = 0; i < glogger->type_size; ++i) {
		struct ct_log_data* data = &glogger->datas[i];
		if (data->storage) { ct_log_storage_schedule(data->storage, tick); }
		if (data->callback) { ct_log_callback_schedule(data->callback); }
	}
}
...
void ct_log_flush(void) {
	...
	for (size_t i = 0; i < glogger->type_size; ++i) {
		struct ct_log_data* data = &glogger->datas[i];
		if (data->storage) { ct_log_storage_flush(data->storage); }
		if (data->callback) { ct_log_callback_flush(data->callback); }
	}
}
...
void ct_log_handle(size_t type_id, int level, const char* buf, size_t size) {
	...
	if (data->storage) { ct_log_storage_handle(data->storage, buf, size); }
	if (data->callback) { ct_log_callback_handle(data->callback, buf, size); }
}
