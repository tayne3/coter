...
#if defined(_LIBCPP_VERSION) && __cplusplus == CT_CXX_11
...
// no constexpr for GCC 4/5 bug
    ...
    template <class T, class E,
              bool = is_void_or<T, CT_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)>::value &&
                     CT_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(E)::value,
...
// move constructible
    ...
    template <class T, class E,
              bool = is_void_or<T, conjunction<CT_EXPECTED_IS_TRIVIALLY_COPY_ASSIGNABLE(T),
                                               CT_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T),
                                               CT_EXPECTED_IS_TRIVIALLY_DESTRUCTIBLE(T)> >::value &&
                     CT_EXPECTED_IS_TRIVIALLY_COPY_ASSIGNABLE(E)::value &&
                     CT_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(E)::value &&
...
// move assignable
    ...
    template <class T, class E,
              bool =
                  is_void_or<T, conjunction<std::is_trivially_destructible<T>, std::is_trivially_move_constructible<T>,
...
public:
...
#if defined(CT_EXPECTED_CXX14) && !defined(CT_EXPECTED_GCC49) && !defined(CT_EXPECTED_GCC54) && \
    !defined(CT_EXPECTED_GCC55)
    template <class F>
...
#else
    template <class F>
    CT_EXPECTED_11_CONSTEXPR auto and_then(F&& f) & -> decltype(and_then_impl(std::declval<expected&>(),
                                                                              std::forward<F>(f))) {
        return and_then_impl(*this, std::forward<F>(f));
    }
    template <class F>
...
#else
    template <class F>
    CT_EXPECTED_11_CONSTEXPR decltype(expected_map_impl(std::declval<expected&>(),
                                                        std::declval<F&&>())) transform(F&& f) & {
        return expected_map_impl(*this, std::forward<F>(f));
    }
    template <class F>
    CT_EXPECTED_11_CONSTEXPR decltype(expected_map_impl(std::declval<expected>(), std::declval<F&&>()))
    transform(F&& f) && {
        return expected_map_impl(std::move(*this), std::forward<F>(f));
    }
    template <class F>
...
#else
    template <class F>
    CT_EXPECTED_11_CONSTEXPR decltype(map_error_impl(std::declval<expected&>(),
                                                     std::declval<F&&>())) map_error(F&& f) & {
        return map_error_impl(*this, std::forward<F>(f));
    }
    template <class F>
    CT_EXPECTED_11_CONSTEXPR decltype(map_error_impl(std::declval<expected&&>(), std::declval<F&&>()))
    map_error(F&& f) && {
        return map_error_impl(std::move(*this), std::forward<F>(f));
    }
    template <class F>
    ...
    constexpr decltype(map_error_impl(std::declval<const expected&&>(),
                                      std::declval<F&&>())) transform_error(F&& f) const&& {
        return map_error_impl(std::move(*this), std::forward<F>(f));
    }
#endif
#endif
    ...
    explicit CT_EXPECTED_11_CONSTEXPR expected(const expected<U, G>& rhs)
