...
#define CT_EXPECTED_EXCEPTIONS_ENABLED
#endif

#if (defined(_MSC_VER) && _MSC_VER == 1900)
#define CT_EXPECTED_MSVC2015
#define CT_EXPECTED_MSVC2015_CONSTEXPR
#else
#define CT_EXPECTED_MSVC2015_CONSTEXPR constexpr
#endif

#if (defined(__GNUC__) && __GNUC__ == 4 && __GNUC_MINOR__ <= 9 && !defined(__clang__))
#define CT_EXPECTED_GCC49
#endif

#if (defined(__GNUC__) && __GNUC__ == 5 && __GNUC_MINOR__ <= 4 && !defined(__clang__))
#define CT_EXPECTED_GCC54
#endif

#if (defined(__GNUC__) && __GNUC__ == 5 && __GNUC_MINOR__ <= 5 && !defined(__clang__))
#define CT_EXPECTED_GCC55
#endif
...
#if (CT_CPLUSPLUS > 201103L) && !defined(CT_EXPECTED_GCC49)
#include <cassert>
...
#define CT_EXPECTED_NO_CONSTRR
// GCC < 5 doesn't support some standard C++11 type traits
#define CT_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) std::has_trivial_copy_constructor<T>
#define CT_EXPECTED_IS_TRIVIALLY_COPY_ASSIGNABLE(T)    std::has_trivial_copy_assign<T>
...
#define CT_EXPECTED_IS_TRIVIALLY_DESTRUCTIBLE(T)       std::is_trivially_destructible<T>
#else
#define CT_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) std::is_trivially_copy_constructible<T>
#define CT_EXPECTED_IS_TRIVIALLY_COPY_ASSIGNABLE(T)    std::is_trivially_copy_assignable<T>
#define CT_EXPECTED_IS_TRIVIALLY_DESTRUCTIBLE(T)       std::is_trivially_destructible<T>
#endif

#if CT_CPLUSPLUS > 201103L
#define CT_EXPECTED_CXX14
#endif

#ifdef CT_EXPECTED_GCC49
#define CT_EXPECTED_GCC49_CONSTEXPR
#else
#define CT_EXPECTED_GCC49_CONSTEXPR constexpr
#endif

#if (CT_CPLUSPLUS == 201103L || defined(CT_EXPECTED_MSVC2015) || defined(CT_EXPECTED_GCC49))
#define CT_EXPECTED_11_CONSTEXPR
#else
#define CT_EXPECTED_11_CONSTEXPR constexpr
#endif

#if CT_CPLUSPLUS >= 201703L
...
public:
	...
	CT_EXPECTED_11_CONSTEXPR E  &value()  &{ return m_val; }
	CT_EXPECTED_11_CONSTEXPR E &&value() && { return std::move(m_val); }
...
#ifdef CT_EXPECTED_EXCEPTIONS_ENABLED
#define CT_EXPECTED_THROW_EXCEPTION(e) throw((e));
#else
#define CT_EXPECTED_THROW_EXCEPTION(e) std::terminate();
#endif

namespace detail {
#ifndef CT_TRAITS_MUTEX
#define CT_TRAITS_MUTEX
...
#define CT_TRAITS_LIBCXX_MEM_FN_WORKAROUND
#endif

// In C++11 mode, there's an issue in libc++'s std::mem_fn
// which results in a hard-error when using it in a noexcept expression
// in some cases. This is a check to workaround the common failing case.
#ifdef CT_TRAITS_LIBCXX_MEM_FN_WORKAROUND
	template <class T>
	...
	template <class T, class U = T>
	struct is_nothrow_swappable : std::integral_constant<bool, is_swappable<T, U>::value && ((decltype(detail::swap_adl_tests::uses_std<T, U>(0))::value &&
	...
	using expected_enable_from_other =
		detail::enable_if_t<std::is_constructible<T, UR>::value && std::is_constructible<E, GR>::value && !std::is_constructible<T, expected<U, G> &>::value &&
...
namespace detail {
	...
	struct expected_storage_base<T, E, true, false> {
		constexpr expected_storage_base() : m_val(T{}), m_has_val(true) {}
		CT_EXPECTED_MSVC2015_CONSTEXPR expected_storage_base(no_init_t) : m_no_init(), m_has_val(false) {}
...
// no constexpr for GCC 4/5 bug
#else
		CT_EXPECTED_MSVC2015_CONSTEXPR
#endif
	...
	struct expected_operations_base : expected_storage_base<T, E> {
		...
		CT_EXPECTED_11_CONSTEXPR T  &get()  &{ return this->m_val; }
		constexpr const T           &get() const           &{ return this->m_val; }
		CT_EXPECTED_11_CONSTEXPR T &&get() && { return std::move(this->m_val); }
#ifndef CT_EXPECTED_NO_CONSTRR
		constexpr const T &&get() const && { return std::move(this->m_val); }
#endif

		CT_EXPECTED_11_CONSTEXPR unexpected<E> &geterr() & { return this->m_unexpect; }
		constexpr const unexpected<E>          &geterr() const          &{ return this->m_unexpect; }
		CT_EXPECTED_11_CONSTEXPR unexpected<E> &&geterr() && { return std::move(this->m_unexpect); }
#ifndef CT_EXPECTED_NO_CONSTRR
		constexpr const unexpected<E> &&geterr() const && { return std::move(this->m_unexpect); }
#endif

		CT_EXPECTED_11_CONSTEXPR void destroy_val() { get().~T(); }
	};
	...
	struct expected_operations_base<void, E> : expected_storage_base<void, E> {
		...
		CT_EXPECTED_11_CONSTEXPR void destroy_val() {
			// no-op
		}
	};
	...
	template <class T, class E,
			  bool = is_void_or<T, CT_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)>::value && CT_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(E)::value,
...
// move constructible
	...
	template <class T, class E,
			  bool = is_void_or<T, conjunction<CT_EXPECTED_IS_TRIVIALLY_COPY_ASSIGNABLE(T), CT_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T),
											   CT_EXPECTED_IS_TRIVIALLY_DESTRUCTIBLE(T)> >::value &&
					 CT_EXPECTED_IS_TRIVIALLY_COPY_ASSIGNABLE(E)::value && CT_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(E)::value &&
					 CT_EXPECTED_IS_TRIVIALLY_DESTRUCTIBLE(E)::value,
			  bool = (is_copy_constructible_or_void<T>::value && std::is_copy_constructible<E>::value && is_copy_assignable_or_void<T>::value &&
...
template <class T, class E>
class CT_EXPECTED_NODISCARD expected : private detail::expected_move_assign_base<T, E>,
	...
	CT_EXPECTED_11_CONSTEXPR U &val() {
		return this->m_val;
	}
	CT_EXPECTED_11_CONSTEXPR unexpected<E> &err() { return this->m_unexpect; }
...
public:
...
#if defined(CT_EXPECTED_CXX14) && !defined(CT_EXPECTED_GCC49) && !defined(CT_EXPECTED_GCC54) && !defined(CT_EXPECTED_GCC55)
	template <class F>
	CT_EXPECTED_11_CONSTEXPR auto and_then(F &&f) & {
...
#else
	template <class F>
	CT_EXPECTED_11_CONSTEXPR auto and_then(F &&f) & -> decltype(and_then_impl(std::declval<expected &>(), std::forward<F>(f))) {
		return and_then_impl(*this, std::forward<F>(f));
	}
	template <class F>
	CT_EXPECTED_11_CONSTEXPR auto and_then(F &&f) && -> decltype(and_then_impl(std::declval<expected &&>(), std::forward<F>(f))) {
...
#if defined(CT_EXPECTED_CXX14) && !defined(CT_EXPECTED_GCC49) && !defined(CT_EXPECTED_GCC54) && !defined(CT_EXPECTED_GCC55)
	template <class F>
	CT_EXPECTED_11_CONSTEXPR auto map(F &&f) & {
...
#else
	template <class F>
	CT_EXPECTED_11_CONSTEXPR decltype(expected_map_impl(std::declval<expected &>(), std::declval<F &&>())) map(F &&f) & {
		return expected_map_impl(*this, std::forward<F>(f));
	}
	template <class F>
	CT_EXPECTED_11_CONSTEXPR decltype(expected_map_impl(std::declval<expected>(), std::declval<F &&>())) map(F &&f) && {
...
#if defined(CT_EXPECTED_CXX14) && !defined(CT_EXPECTED_GCC49) && !defined(CT_EXPECTED_GCC54) && !defined(CT_EXPECTED_GCC55)
	template <class F>
	CT_EXPECTED_11_CONSTEXPR auto transform(F &&f) & {
		return expected_map_impl(*this, std::forward<F>(f));
	}
	template <class F>
	CT_EXPECTED_11_CONSTEXPR auto transform(F &&f) && {
...
#else
	template <class F>
	CT_EXPECTED_11_CONSTEXPR decltype(expected_map_impl(std::declval<expected &>(), std::declval<F &&>())) transform(F &&f) & {
		return expected_map_impl(*this, std::forward<F>(f));
	}
	template <class F>
	CT_EXPECTED_11_CONSTEXPR decltype(expected_map_impl(std::declval<expected>(), std::declval<F &&>())) transform(F &&f) && {
...
#if defined(CT_EXPECTED_CXX14) && !defined(CT_EXPECTED_GCC49) && !defined(CT_EXPECTED_GCC54) && !defined(CT_EXPECTED_GCC55)
	template <class F>
	CT_EXPECTED_11_CONSTEXPR auto map_error(F &&f) & {
...
#else
	template <class F>
	CT_EXPECTED_11_CONSTEXPR decltype(map_error_impl(std::declval<expected &>(), std::declval<F &&>())) map_error(F &&f) & {
		return map_error_impl(*this, std::forward<F>(f));
	}
	template <class F>
	CT_EXPECTED_11_CONSTEXPR decltype(map_error_impl(std::declval<expected &&>(), std::declval<F &&>())) map_error(F &&f) && {
...
#if defined(CT_EXPECTED_CXX14) && !defined(CT_EXPECTED_GCC49) && !defined(CT_EXPECTED_GCC54) && !defined(CT_EXPECTED_GCC55)
	template <class F>
	CT_EXPECTED_11_CONSTEXPR auto transform_error(F &&f) & {
		return map_error_impl(*this, std::forward<F>(f));
	}
	template <class F>
	CT_EXPECTED_11_CONSTEXPR auto transform_error(F &&f) && {
...
#else
	template <class F>
	CT_EXPECTED_11_CONSTEXPR decltype(map_error_impl(std::declval<expected &>(), std::declval<F &&>())) transform_error(F &&f) & {
		return map_error_impl(*this, std::forward<F>(f));
	}
	template <class F>
	CT_EXPECTED_11_CONSTEXPR decltype(map_error_impl(std::declval<expected &&>(), std::declval<F &&>())) transform_error(F &&f) && {
...
#endif
	template <class F>
	expected CT_EXPECTED_11_CONSTEXPR or_else(F &&f) & {
	...
	template <class... Args, detail::enable_if_t<std::is_constructible<T, Args &&...>::value> * = nullptr>
	constexpr expected(in_place_t, Args &&...args) : impl_base(in_place, std::forward<Args>(args)...), ctor_base(detail::default_constructor_tag{}) {}
	...
	template <class... Args, detail::enable_if_t<std::is_constructible<E, Args &&...>::value> * = nullptr>
	constexpr explicit expected(unexpect_t, Args &&...args) : impl_base(unexpect, std::forward<Args>(args)...), ctor_base(detail::default_constructor_tag{}) {}
	...
	explicit CT_EXPECTED_11_CONSTEXPR expected(const expected<U, G> &rhs) : ctor_base(detail::default_constructor_tag{}) {
		if (rhs.has_value()) {
			this->construct(*rhs);
		} else {
	...
	template <class U, class G, detail::enable_if_t<(std::is_convertible<U const &, T>::value && std::is_convertible<G const &, E>::value)> * = nullptr,
			  detail::expected_enable_from_other<T, E, U, G, const U &, const G &> * = nullptr>
	CT_EXPECTED_11_CONSTEXPR expected(const expected<U, G> &rhs) : ctor_base(detail::default_constructor_tag{}) {
		if (rhs.has_value()) {
			this->construct(*rhs);
		} else {
			this->construct_error(rhs.error());
	...
	explicit CT_EXPECTED_11_CONSTEXPR expected(expected<U, G> &&rhs) : ctor_base(detail::default_constructor_tag{}) {
		if (rhs.has_value()) {
	...
	template <class U, class G, detail::enable_if_t<(std::is_convertible<U &&, T>::value && std::is_convertible<G &&, E>::value)> * = nullptr,
			  detail::expected_enable_from_other<T, E, U, G, U &&, G &&> * = nullptr>
	CT_EXPECTED_11_CONSTEXPR expected(expected<U, G> &&rhs) : ctor_base(detail::default_constructor_tag{}) {
		if (rhs.has_value()) {
			this->construct(std::move(*rhs));
		} else {
			this->construct_error(std::move(rhs.error()));
	...
	explicit CT_EXPECTED_MSVC2015_CONSTEXPR expected(U &&v) : expected(in_place, std::forward<U>(v)) {}

	template <class U = T, detail::enable_if_t<std::is_convertible<U &&, T>::value> * = nullptr, detail::expected_enable_forward_value<T, E, U> * = nullptr>
	CT_EXPECTED_MSVC2015_CONSTEXPR expected(U &&v) : expected(in_place, std::forward<U>(v)) {}
...
public:
	...
	CT_EXPECTED_11_CONSTEXPR T *operator->() {
		CT_ASSERT(has_value());
		return valptr();
	}
	...
	template <class U = T, detail::enable_if_t<!std::is_void<U>::value> * = nullptr>
	CT_EXPECTED_11_CONSTEXPR U &operator*() & {
		CT_ASSERT(has_value());
		return val();
	}
	template <class U = T, detail::enable_if_t<!std::is_void<U>::value> * = nullptr>
	...
	template <class U = T, detail::enable_if_t<!std::is_void<U>::value> * = nullptr>
	CT_EXPECTED_11_CONSTEXPR const U &value() const & {
		if (!has_value()) CT_EXPECTED_THROW_EXCEPTION(bad_expected_access<E>(err().value()));
		return val();
	}
	template <class U = T, detail::enable_if_t<!std::is_void<U>::value> * = nullptr>
	CT_EXPECTED_11_CONSTEXPR U &value() & {
		if (!has_value()) CT_EXPECTED_THROW_EXCEPTION(bad_expected_access<E>(err().value()));
		return val();
	}
	template <class U = T, detail::enable_if_t<!std::is_void<U>::value> * = nullptr>
	CT_EXPECTED_11_CONSTEXPR const U &&value() const && {
		if (!has_value()) CT_EXPECTED_THROW_EXCEPTION(bad_expected_access<E>(std::move(err()).value()));
		return std::move(val());
	}
	template <class U = T, detail::enable_if_t<!std::is_void<U>::value> * = nullptr>
	CT_EXPECTED_11_CONSTEXPR U &&value() && {
	...
	CT_EXPECTED_11_CONSTEXPR E &error() & {
		CT_ASSERT(!has_value());
		return err().value();
	}
	constexpr const E &&error() const && {
	...
	template <class U>
	CT_EXPECTED_11_CONSTEXPR T value_or(U &&v) && {
