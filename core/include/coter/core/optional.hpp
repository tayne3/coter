...
#define CT_OPTIONAL_MSVC2015
#endif

#if (defined(__GNUC__) && __GNUC__ == 4 && __GNUC_MINOR__ <= 9 && !defined(__clang__))
#define CT_OPTIONAL_GCC49
#endif

#if (defined(__GNUC__) && __GNUC__ == 5 && __GNUC_MINOR__ <= 4 && !defined(__clang__))
#define CT_OPTIONAL_GCC54
#endif

#if (defined(__GNUC__) && __GNUC__ == 5 && __GNUC_MINOR__ <= 5 && !defined(__clang__))
#define CT_OPTIONAL_GCC55
#endif

#if (defined(__GNUC__) && __GNUC__ == 4 && __GNUC_MINOR__ <= 9 && !defined(__clang__))
// GCC < 5 doesn't support overloading on const&& for member functions
#define CT_OPTIONAL_NO_CONSTRR

// GCC < 5 doesn't support some standard C++11 type traits
#define CT_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) std::has_trivial_copy_constructor<T>::value
#define CT_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T)    std::has_trivial_copy_assign<T>::value
...
#define CT_OPTIONAL_IS_TRIVIALLY_DESTRUCTIBLE(T)       std::is_trivially_destructible<T>::value
#else
#define CT_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) std::is_trivially_copy_constructible<T>::value
#define CT_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T)    std::is_trivially_copy_assignable<T>::value
#define CT_OPTIONAL_IS_TRIVIALLY_DESTRUCTIBLE(T)       std::is_trivially_destructible<T>::value
#endif

#if __cplusplus > 201103L
#define CT_OPTIONAL_CXX14
#endif

// constexpr implies const in C++11, not C++14
#if (__cplusplus == 201103L || defined(CT_OPTIONAL_MSVC2015) || defined(CT_OPTIONAL_GCC49))
#define CT_OPTIONAL_11_CONSTEXPR
#else
...
// in some cases. This is a check to workaround the common failing case.
	...
	struct optional_storage_base {
		CT_OPTIONAL_11_CONSTEXPR optional_storage_base() noexcept : m_dummy(), m_has_value(false) {}

		template <class... U>
		CT_OPTIONAL_11_CONSTEXPR optional_storage_base(in_place_t, U &&...u) : m_value(std::forward<U>(u)...), m_has_value(true) {}
	...
	struct optional_operations_base : optional_storage_base<T> {
		...
		CT_OPTIONAL_11_CONSTEXPR T       &get()       &{ return this->m_value; }
		CT_OPTIONAL_11_CONSTEXPR const T &get() const & { return this->m_value; }
		CT_OPTIONAL_11_CONSTEXPR T      &&get()      &&{ return std::move(this->m_value); }
#ifndef CT_OPTIONAL_NO_CONSTRR
	...
	template <class T, bool = CT_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)>
...
// move constructible
	...
	template <class T,
			  bool = CT_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T) && CT_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) && CT_OPTIONAL_IS_TRIVIALLY_DESTRUCTIBLE(T)>
...
/// the optional object.
template <class T>
class optional : private detail::optional_move_assign_base<T>, private detail::optional_delete_ctor_base<T>, private detail::optional_delete_assign_base<T> {
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
#if defined(CT_OPTIONAL_CXX14) && !defined(CT_OPTIONAL_GCC49) && !defined(CT_OPTIONAL_GCC54) && !defined(CT_OPTIONAL_GCC55)
	/// Carries out some operation which returns an optional on the stored
	/// object if there is one.
	template <class F>
	CT_OPTIONAL_11_CONSTEXPR auto and_then(F &&f) & {
	...
	/// object if there is one.
	template <class F>
	CT_OPTIONAL_11_CONSTEXPR detail::invoke_result_t<F, T &> and_then(F &&f) & {
		using result = detail::invoke_result_t<F, T &>;
		static_assert(detail::is_optional<result>::value, "F must return an optional");
	...
	template <class F>
	CT_OPTIONAL_11_CONSTEXPR auto map(F &&f) & {
	...
	/// Carries out some operation on the stored object if there is one.
	template <class F>
	CT_OPTIONAL_11_CONSTEXPR decltype(optional_map_impl(std::declval<optional &>(), std::declval<F &&>())) map(F &&f) & {
		return optional_map_impl(*this, std::forward<F>(f));
	}
	...
	/// Carries out some operation on the stored object if there is one.
	template <class F>
	CT_OPTIONAL_11_CONSTEXPR auto transform(F &&f) & {
		return optional_map_impl(*this, std::forward<F>(f));
	}
	...
	/// Carries out some operation on the stored object if there is one.
	template <class F>
	CT_OPTIONAL_11_CONSTEXPR decltype(optional_map_impl(std::declval<optional &>(), std::declval<F &&>())) transform(F &&f) & {
		return optional_map_impl(*this, std::forward<F>(f));
	}
	...
	optional<T> CT_OPTIONAL_11_CONSTEXPR or_else(F &&f) & {
	...
	template <class F, detail::disable_if_ret_void<F> * = nullptr>
	optional<T> CT_OPTIONAL_11_CONSTEXPR or_else(F &&f) const & {
		return has_value() ? *this : std::forward<F>(f)();
	}
	...
	CT_OPTIONAL_11_CONSTEXPR optional disjunction(const optional &rhs) & { return has_value() ? *this : rhs; }

	constexpr optional disjunction(const optional &rhs) const & { return has_value() ? *this : rhs; }

	CT_OPTIONAL_11_CONSTEXPR optional disjunction(const optional &rhs) && { return has_value() ? std::move(*this) : rhs; }

#ifndef CT_OPTIONAL_NO_CONSTRR
	constexpr optional disjunction(const optional &rhs) const && { return has_value() ? std::move(*this) : rhs; }
#endif

	CT_OPTIONAL_11_CONSTEXPR optional disjunction(optional &&rhs) & { return has_value() ? *this : std::move(rhs); }

	constexpr optional disjunction(optional &&rhs) const & { return has_value() ? *this : std::move(rhs); }

	CT_OPTIONAL_11_CONSTEXPR optional disjunction(optional &&rhs) && { return has_value() ? std::move(*this) : std::move(rhs); }
	...
	CT_OPTIONAL_11_CONSTEXPR optional(const optional &rhs) = default;

	/// Move constructor
	///
	/// If `rhs` contains a value, the stored value is direct-initialized with
	/// it. Otherwise, the constructed optional is empty.
	CT_OPTIONAL_11_CONSTEXPR optional(optional &&rhs) = default;
	...
	CT_OPTIONAL_11_CONSTEXPR explicit optional(detail::enable_if_t<std::is_constructible<T, std::initializer_list<U> &, Args &&...>::value, in_place_t>,
	...
	CT_OPTIONAL_11_CONSTEXPR T *operator->() { return std::addressof(this->m_value); }

	/// Returns the stored value
	CT_OPTIONAL_11_CONSTEXPR T &operator*() & { return this->m_value; }

	constexpr const T &operator*() const & { return this->m_value; }

	CT_OPTIONAL_11_CONSTEXPR T &&operator*() && { return std::move(this->m_value); }
	...
	CT_OPTIONAL_11_CONSTEXPR T &value() & {
		if (has_value()) return this->m_value;
		throw bad_optional_access();
	}
	CT_OPTIONAL_11_CONSTEXPR const T &value() const & {
		if (has_value()) return this->m_value;
		throw bad_optional_access();
	}
	CT_OPTIONAL_11_CONSTEXPR T &&value() && {
	...
	template <class U>
	CT_OPTIONAL_11_CONSTEXPR T value_or(U &&u) && {
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
	...
	/// Carries out some operation on the stored object if there is one.
	template <class F>
	CT_OPTIONAL_11_CONSTEXPR decltype(detail::optional_map_impl(std::declval<optional &>(), std::declval<F &&>())) map(F &&f) & {
		return detail::optional_map_impl(*this, std::forward<F>(f));
	}
	...
	/// Carries out some operation on the stored object if there is one.
	template <class F>
	CT_OPTIONAL_11_CONSTEXPR decltype(detail::optional_map_impl(std::declval<optional &>(), std::declval<F &&>())) transform(F &&f) & {
		return detail::optional_map_impl(*this, std::forward<F>(f));
	}
	...
	/// If `rhs` contains a value, the stored value is direct-initialized with
	/// it. Otherwise, the constructed optional is empty.
	CT_OPTIONAL_11_CONSTEXPR optional(const optional &rhs) noexcept = default;
	...
	CT_OPTIONAL_11_CONSTEXPR T *operator->() noexcept { return m_value; }
	...
	/// \group value_or
	template <class U>
	CT_OPTIONAL_11_CONSTEXPR T value_or(U &&u) && noexcept {
		static_assert(std::is_move_constructible<T>::value && std::is_convertible<U &&, T>::value, "T must be move constructible and convertible from U");
		return has_value() ? **this : static_cast<T>(std::forward<U>(u));
	}
