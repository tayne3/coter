...
#if __cplusplus > CT_CXX_11
#define CT_OPTIONAL_CXX14
#endif

// constexpr implies const in C++11, not C++14
#if (__cplusplus == CT_CXX_11 || defined(CT_OPTIONAL_MSVC2015) || defined(CT_OPTIONAL_GCC49))
...
// in some cases. This is a check to workaround the common failing case.
    ...
    struct optional_storage_base {
        ...
        template <class... U>
        CT_OPTIONAL_11_CONSTEXPR optional_storage_base(in_place_t, U&&... u)
...
// move constructible
    ...
    template <class T, bool = CT_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T) &&
                              CT_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) &&
                              CT_OPTIONAL_IS_TRIVIALLY_DESTRUCTIBLE(T)>
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
#if defined(CT_OPTIONAL_CXX14) && !defined(CT_OPTIONAL_GCC49) && !defined(CT_OPTIONAL_GCC54) && \
    !defined(CT_OPTIONAL_GCC55)
    ...
    template <class F>
    CT_OPTIONAL_11_CONSTEXPR decltype(optional_map_impl(std::declval<optional&&>(), std::declval<F&&>()))
    map(F&& f) && {
        return optional_map_impl(std::move(*this), std::forward<F>(f));
    }
    ...
    CT_OPTIONAL_11_CONSTEXPR optional disjunction(const optional& rhs) && {
    ...
    CT_OPTIONAL_11_CONSTEXPR optional disjunction(optional&& rhs) && {
    ...
    CT_OPTIONAL_11_CONSTEXPR explicit optional(
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
    ...
    /// Carries out some operation on the stored object if there is one.
    template <class F>
    CT_OPTIONAL_11_CONSTEXPR decltype(detail::optional_map_impl(std::declval<optional&>(), std::declval<F&&>()))
    map(F&& f) & {
        return detail::optional_map_impl(*this, std::forward<F>(f));
    }
