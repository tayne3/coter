...
static void test_packet_clean(void) {
	...
	for (uint16_t i = 0; i < 50; ++i) { assert_uint8_eq(0, buffer[i]); }
}
...
static void test_packet_u8s(void) {
...

	{
		...
		for (uint16_t i = 0; i < 5; ++i) { assert_uint8_eq(test_array[i], read_array[i]); }
	}
...

	{
		...
		for (uint16_t i = 0; i < 5; ++i) { assert_uint8_eq(test_array[i], read_array[i]); }
		for (uint16_t i = 0; i < 5; ++i) { assert_uint8_eq(test_array[i], read_array[i + 5]); }
	}
	...
	for (uint16_t start = 0; start < 10; start++) {
		for (uint16_t end = start + 1; end < 10; end++) {
			...
			for (uint16_t i = 0; i < read_length; ++i) { assert_uint8_eq(test_array[(start + i) % 5], read_array[i]); }
		}
	}
...
static void test_packet_u16s(void) {
	...
	}
	{
		...
		for (uint16_t i = 0; i < 5; ++i) { assert_uint16_eq(test_array[i], read_array[i]); }
	}
	...
	}
	{
		...
		for (uint16_t i = 0; i < 5; ++i) { assert_uint16_eq(test_array[i], read_array[i + 5]); }
	}
	...
	for (uint16_t start = 0; start < 10; start++) {
		for (uint16_t end = start + 1; end < 10; end++) {
			if (start < 5) {
				memset(read_array, 0, sizeof(read_array));
				const uint16_t read_length = end - start;
				const uint16_t read_result = ct_packet_get_u16s(packet, start << 1, read_array, read_length, CT_ENDIAN_BIG);
				assert_uint16_eq(read_length, read_result);
				assert_uint16_eq(20, ct_packet_size(packet));
				assert_uint16_eq(20, ct_packet_total_size(packet));
				assert_uint16_eq(0, ct_packet_past(packet));
				...
				for (uint16_t i = 0; i < read_length && start + i < 5; ++i) {
					assert_uint16_eq(test_array[start + i], read_array[i], "start = %u, end = %u, read_length = %u, i = %u\n", start, end, read_length, i);
				}
			}
			if (end >= 5) {
				memset(read_array, 0, sizeof(read_array));
				const uint16_t read_length = end - start;
				const uint16_t read_result = ct_packet_get_u16s(packet, start << 1, read_array, read_length, CT_ENDIAN_LITTLE);
				assert_uint16_eq(read_length, read_result);
				assert_uint16_eq(20, ct_packet_size(packet));
				assert_uint16_eq(20, ct_packet_total_size(packet));
				assert_uint16_eq(0, ct_packet_past(packet));
				...
				for (uint16_t i = 0; i < read_length; ++i) {
					...
					assert_uint16_eq(test_array[(start + i) - 5], read_array[i], "start = %u, end = %u, read_length = %u, i = %u\n", start, end, read_length,
									 i);
				}
			}
		}
...
static void test_packet_u32s(void) {
	...
	}
	{
		...
		for (uint16_t i = 0; i < 5; ++i) { assert_uint32_eq(test_array[i], read_array[i]); }
	}
	...
	}
	{
		...
		for (uint16_t i = 0; i < 5; ++i) { assert_uint32_eq(test_array[i], read_array[i + 5]); }
	}
	...
	for (uint16_t start = 0; start < 10; start++) {
		for (uint16_t end = start + 1; end < 10; end++) {
			if (start < 5) {
				memset(read_array, 0, sizeof(read_array));
				const uint16_t read_length = end - start;
				const uint16_t read_result = ct_packet_get_u32s(packet, start << 2, read_array, read_length, CT_ENDIAN_BIG);
				assert_uint16_eq(read_length, read_result);
				assert_uint16_eq(40, ct_packet_size(packet));
				assert_uint16_eq(40, ct_packet_total_size(packet));
				assert_uint16_eq(0, ct_packet_past(packet));
				...
				for (uint16_t i = 0; i < read_length && start + i < 5; ++i) {
					assert_uint32_eq(test_array[start + i], read_array[i], "start = %u, end = %u, read_length = %u, i = %u\n", start, end, read_length, i);
				}
			}
			if (end >= 5) {
				memset(read_array, 0, sizeof(read_array));
				const uint16_t read_length = end - start;
				const uint16_t read_result = ct_packet_get_u32s(packet, start << 2, read_array, read_length, CT_ENDIAN_LITTLE);
				assert_uint16_eq(read_length, read_result);
				assert_uint16_eq(40, ct_packet_size(packet));
				assert_uint16_eq(40, ct_packet_total_size(packet));
				assert_uint16_eq(0, ct_packet_past(packet));
				...
				for (uint16_t i = 0; i < read_length; ++i) {
					if (start + i < 5) { continue; }
					assert_uint32_eq(test_array[(start + i) - 5], read_array[i], "start = %u, end = %u, read_length = %u, i = %u\n", start, end, read_length,
									 i);
				}
			}
		}
...
static void test_packet_u64s(void) {
	...
	}
	{
		...
		for (uint16_t i = 0; i < 5; ++i) { assert_uint64_eq(test_array[i], read_array[i]); }
	}
	...
	for (uint16_t start = 0; start < 10; start++) {
		for (uint16_t end = start + 1; end < 10; end++) {
			if (start < 5) {
				memset(read_array, 0, sizeof(read_array));
				const uint16_t read_length = end - start;
				const uint16_t read_result = ct_packet_get_u64s(packet, start << 3, read_array, read_length, CT_ENDIAN_BIG);
				assert_uint16_eq(read_length, read_result);
				assert_uint16_eq(80, ct_packet_size(packet));
				assert_uint16_eq(80, ct_packet_total_size(packet));
				assert_uint16_eq(0, ct_packet_past(packet));
				...
				for (uint16_t i = 0; i < read_length && start + i < 5; ++i) {
					assert_uint64_eq(test_array[start + i], read_array[i], "start = %u, end = %u, read_length = %u, i = %u\n", start, end, read_length, i);
				}
			}
			if (end >= 5) {
				memset(read_array, 0, sizeof(read_array));
				const uint16_t read_length = end - start;
				const uint16_t read_result = ct_packet_get_u64s(packet, start << 3, read_array, read_length, CT_ENDIAN_LITTLE);
				assert_uint16_eq(read_length, read_result);
				assert_uint16_eq(80, ct_packet_size(packet));
				assert_uint16_eq(80, ct_packet_total_size(packet));
				assert_uint16_eq(0, ct_packet_past(packet));
				...
				for (uint16_t i = 0; i < read_length; ++i) {
					if (start + i < 5) { continue; }
					assert_uint64_eq(test_array[(start + i) - 5], read_array[i], "start = %u, end = %u, read_length = %u, i = %u\n", start, end, read_length,
									 i);
				}
			}
		}
	...
	// take: big endian
	{
		...
		for (uint16_t i = 0; i < 5; ++i) { assert_uint64_eq(test_array[i], taken_array[i]); }
	}
...
static void test_packet_floats(void) {
	...
	}
	{
		...
		for (uint16_t i = 0; i < 5; ++i) { assert_float_eq(test_array[i], read_array[i]); }
	}
	...
	}
	{
		...
		for (uint16_t i = 0; i < 5; ++i) { assert_float_eq(test_array[i], read_array[i + 5]); }
	}
	...
	for (uint16_t start = 0; start < 10; start++) {
		for (uint16_t end = start + 1; end < 10; end++) {
			if (start < 5) {
				memset(read_array, 0, sizeof(read_array));
				const uint16_t read_length = end - start;
				const uint16_t read_result = ct_packet_get_floats(packet, start << 2, read_array, read_length, CT_ENDIAN_BIG);
				assert_uint16_eq(read_length, read_result);
				assert_uint16_eq(40, ct_packet_size(packet));
				assert_uint16_eq(40, ct_packet_total_size(packet));
				assert_uint16_eq(0, ct_packet_past(packet));
				...
				for (uint16_t i = 0; i < read_length && start + i < 5; ++i) {
					assert_float_eq(test_array[start + i], read_array[i], "start = %u, end = %u, read_length = %u, i = %u\n", start, end, read_length, i);
				}
			}
			if (end >= 5) {
				memset(read_array, 0, sizeof(read_array));
				const uint16_t read_length = end - start;
				const uint16_t read_result = ct_packet_get_floats(packet, start << 2, read_array, read_length, CT_ENDIAN_LITTLE);
				assert_uint16_eq(read_length, read_result);
				assert_uint16_eq(40, ct_packet_size(packet));
				assert_uint16_eq(40, ct_packet_total_size(packet));
				assert_uint16_eq(0, ct_packet_past(packet));
				...
				for (uint16_t i = 0; i < read_length; ++i) {
					if (start + i < 5) { continue; }
					assert_float_eq(test_array[(start + i) - 5], read_array[i], "start = %u, end = %u, read_length = %u, i = %u\n", start, end, read_length, i);
				}
			}
		}
	}
...
static void test_packet_doubles(void) {
	...
	}
	{
		...
		for (uint16_t i = 0; i < 5; ++i) { assert_double_eq(test_array[i], read_array[i]); }
	}
	...
	for (uint16_t start = 0; start < 10; start++) {
		for (uint16_t end = start + 1; end < 10; end++) {
			if (start < 5) {
				memset(read_array, 0, sizeof(read_array));
				const uint16_t read_length = end - start;
				const uint16_t read_result = ct_packet_get_doubles(packet, start << 3, read_array, read_length, CT_ENDIAN_BIG);
				assert_uint16_eq(read_length, read_result);
				assert_uint16_eq(80, ct_packet_size(packet));
				assert_uint16_eq(80, ct_packet_total_size(packet));
				assert_uint16_eq(0, ct_packet_past(packet));
				...
				for (uint16_t i = 0; i < read_length && start + i < 5; ++i) {
					assert_double_eq(test_array[start + i], read_array[i], "start = %u, end = %u, read_length = %u, i = %u\n", start, end, read_length, i);
				}
			}
			if (end >= 5) {
				memset(read_array, 0, sizeof(read_array));
				const uint16_t read_length = end - start;
				const uint16_t read_result = ct_packet_get_doubles(packet, start << 3, read_array, read_length, CT_ENDIAN_LITTLE);
				assert_uint16_eq(read_length, read_result);
				assert_uint16_eq(80, ct_packet_size(packet));
				assert_uint16_eq(80, ct_packet_total_size(packet));
				assert_uint16_eq(0, ct_packet_past(packet));
				...
				for (uint16_t i = 0; i < read_length; ++i) {
					if (start + i < 5) { continue; }
					assert_double_eq(test_array[(start + i) - 5], read_array[i], "start = %u, end = %u, read_length = %u, i = %u\n", start, end, read_length,
									 i);
				}
			}
		}
	...
	// take: big endian
	{
		...
		for (uint16_t i = 0; i < 5; ++i) { assert_double_eq(test_array[i], taken_array[i]); }
	}
