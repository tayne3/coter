...
uint8_t ct_packet_get_u8(const ct_packet_buf_t self, uint16_t offset) {
	...
	if (offset + self->_past + sizeof(uint8_t) > self->_total) { return 0; }
...
uint16_t ct_packet_get_u16(const ct_packet_buf_t self, uint16_t offset, ct_endian_t endian) {
	...
	if (offset + self->_past + sizeof(uint16_t) > self->_total) { return 0; }
...
uint32_t ct_packet_get_u32(const ct_packet_buf_t self, uint16_t offset, ct_endian_t endian) {
	...
	if (offset + self->_past + sizeof(uint32_t) > self->_total) { return 0; }
...
uint64_t ct_packet_get_u64(const ct_packet_buf_t self, uint16_t offset, ct_endian_t endian) {
	...
	if (offset + self->_past + sizeof(uint64_t) > self->_total) { return 0; }
...
float ct_packet_get_float(const ct_packet_buf_t self, uint16_t offset, ct_endian_t endian) {
	...
	if (offset + self->_past + sizeof(float) > self->_total) { return 0; }
...
double ct_packet_get_double(const ct_packet_buf_t self, uint16_t offset, ct_endian_t endian) {
	...
	if (offset + self->_past + sizeof(double) > self->_total) { return 0; }
...
uint8_t ct_packet_take_u8(ct_packet_buf_t self) {
	...
	if (self->_past + sizeof(uint8_t) > self->_total) { return 0; }
...
uint16_t ct_packet_take_u16(ct_packet_buf_t self, ct_endian_t endian) {
	...
	if (self->_past + sizeof(uint16_t) > self->_total) { return 0; }
...
uint32_t ct_packet_take_u32(ct_packet_buf_t self, ct_endian_t endian) {
	...
	if (self->_past + sizeof(uint32_t) > self->_total) { return 0; }
...
uint64_t ct_packet_take_u64(ct_packet_buf_t self, ct_endian_t endian) {
	...
	if (self->_past + sizeof(uint64_t) > self->_total) { return 0; }
...
float ct_packet_take_float(ct_packet_buf_t self, ct_endian_t endian) {
	...
	if (self->_past + sizeof(float) > self->_total) { return 0; }
...
double ct_packet_take_double(ct_packet_buf_t self, ct_endian_t endian) {
	...
	if (self->_past + sizeof(double) > self->_total) { return 0; }
...
void ct_packet_skip(ct_packet_buf_t self, uint16_t length) {
	...
	if (self->_past + length > self->_total) { return; }
...
void ct_packet_put_u8(ct_packet_buf_t self, uint8_t value) {
	...
	if (self->_total + sizeof(uint8_t) > self->_max) { return; }
...
void ct_packet_put_u16(ct_packet_buf_t self, uint16_t value, ct_endian_t endian) {
	...
	if (self->_total + sizeof(uint16_t) > self->_max) { return; }
...
void ct_packet_put_u32(ct_packet_buf_t self, uint32_t value, ct_endian_t endian) {
	...
	if (self->_total + sizeof(uint32_t) > self->_max) { return; }
...
void ct_packet_put_u64(ct_packet_buf_t self, uint64_t value, ct_endian_t endian) {
	...
	if (self->_total + sizeof(uint64_t) > self->_max) { return; }
...
void ct_packet_put_float(ct_packet_buf_t self, float value, ct_endian_t endian) {
	...
	if (self->_total + sizeof(float) > self->_max) { return; }
...
void ct_packet_put_double(ct_packet_buf_t self, double value, ct_endian_t endian) {
	...
	if (self->_total + sizeof(double) > self->_max) { return; }
...
uint16_t ct_packet_get_u8s(const ct_packet_buf_t self, uint16_t offset, uint8_t *buffer, uint16_t max) {
	...
	if (max + offset + self->_past > self->_total) { max = self->_total - offset - self->_past; }
	memcpy(buffer, self->_buffer + self->_past + offset, max);
	return max;
}

uint16_t ct_packet_get_u16s(const ct_packet_buf_t self, uint16_t offset, uint16_t *buffer, uint16_t max, ct_endian_t endian) {
	if (!self || !self->_buffer) { return 0; }

	if (offset + self->_past + sizeof(uint16_t) > self->_total) { return 0; }

	if ((max << 1) + offset + self->_past > self->_total) { max = (self->_total - offset - self->_past) >> 1; }

	const uint16_t *source = (const uint16_t *)(self->_buffer + self->_past + offset);
	if (endian == CT_ENDIAN_SYSTEM) {
		for (uint16_t i = 0; i < max; ++i) { memcpy(buffer++, source++, sizeof(uint16_t)); }
	} else {
		for (uint16_t i = 0; i < max; ++i) { ct_reverse_memcpy(buffer++, source++, sizeof(uint16_t)); }
	}

	return max;  // 返回读取的元素数量
}

uint16_t ct_packet_get_u32s(const ct_packet_buf_t self, uint16_t offset, uint32_t *buffer, uint16_t max, ct_endian_t endian) {
	if (!self || !self->_buffer) { return 0; }

	if (offset + self->_past + sizeof(uint32_t) > self->_total) { return 0; }

	if ((max << 2) + offset + self->_past > self->_total) { max = (self->_total - offset - self->_past) >> 2; }

	const uint32_t *source = (const uint32_t *)(self->_buffer + self->_past + offset);
	if (endian == CT_ENDIAN_SYSTEM) {
		for (uint16_t i = 0; i < max; ++i) { memcpy(buffer++, source++, sizeof(uint32_t)); }
	} else {
		for (uint16_t i = 0; i < max; ++i) { ct_reverse_memcpy(buffer++, source++, sizeof(uint32_t)); }
	}

	return max;  // 返回读取的元素数量
}

uint16_t ct_packet_get_u64s(const ct_packet_buf_t self, uint16_t offset, uint64_t *buffer, uint16_t max, ct_endian_t endian) {
	if (!self || !self->_buffer) { return 0; }

	if (offset + self->_past + sizeof(uint64_t) > self->_total) { return 0; }

	if ((max << 3) + offset + self->_past > self->_total) { max = (self->_total - offset - self->_past) >> 3; }

	const uint64_t *source = (const uint64_t *)(self->_buffer + self->_past + offset);
	if (endian == CT_ENDIAN_SYSTEM) {
		for (uint16_t i = 0; i < max; ++i) { memcpy(buffer++, source++, sizeof(uint64_t)); }
	} else {
		for (uint16_t i = 0; i < max; ++i) { ct_reverse_memcpy(buffer++, source++, sizeof(uint64_t)); }
	}

	return max;  // 返回读取的元素数量
}

uint16_t ct_packet_get_floats(const ct_packet_buf_t self, uint16_t offset, float *buffer, uint16_t max, ct_endian_t endian) {
	return ct_packet_get_u32s(self, offset, (uint32_t *)buffer, max, endian);
}

uint16_t ct_packet_get_doubles(const ct_packet_buf_t self, uint16_t offset, double *buffer, uint16_t max, ct_endian_t endian) {
...
uint16_t ct_packet_take_u8s(ct_packet_buf_t self, uint8_t *buffer, uint16_t max) {
	...
	if (max + self->_past > self->_total) { max = self->_total - self->_past; }
...
uint16_t ct_packet_take_u16s(ct_packet_buf_t self, uint16_t *buffer, uint16_t max, ct_endian_t endian) {
	...
	if ((max << 1) + self->_past > self->_total) { max = (self->_total - self->_past) >> 1; }
...
uint16_t ct_packet_take_u32s(ct_packet_buf_t self, uint32_t *buffer, uint16_t max, ct_endian_t endian) {
	...
	if ((max << 2) + self->_past > self->_total) { max = (self->_total - self->_past) >> 2; }
...
uint16_t ct_packet_take_u64s(ct_packet_buf_t self, uint64_t *buffer, uint16_t max, ct_endian_t endian) {
	...
	if ((max << 3) + self->_past > self->_total) { max = (self->_total - self->_past) >> 3; }
...
uint16_t ct_packet_put_u8s(ct_packet_buf_t self, const uint8_t *buffer, uint16_t length) {
	...
	if (self->_total + sizeof(uint8_t) > self->_max) { return 0; }

	if (length + self->_total > self->_max) { length = self->_max - self->_total; }
...
uint16_t ct_packet_put_u16s(ct_packet_buf_t self, const uint16_t *buffer, uint16_t length, ct_endian_t endian) {
	...
	if (self->_total + sizeof(uint16_t) > self->_max) { return 0; }

	if ((length << 1) + self->_total > self->_max) { length = (self->_max - self->_total) >> 1; }

	uint16_t *target = (uint16_t *)(self->_buffer + self->_total);
	if (endian == CT_ENDIAN_SYSTEM) {
		for (uint16_t i = 0; i < length; ++i) { memcpy(target++, buffer++, sizeof(uint16_t)); }
	} else {
		for (uint16_t i = 0; i < length; ++i) { ct_reverse_memcpy(target++, buffer++, sizeof(uint16_t)); }
	}
...
uint16_t ct_packet_put_u32s(ct_packet_buf_t self, const uint32_t *buffer, uint16_t length, ct_endian_t endian) {
	...
	if (self->_total + sizeof(uint32_t) > self->_max) { return 0; }

	if ((length << 2) + self->_total > self->_max) { length = (self->_max - self->_total) >> 2; }

	uint32_t *target = (uint32_t *)(self->_buffer + self->_total);
	if (endian == CT_ENDIAN_SYSTEM) {
		for (uint16_t i = 0; i < length; ++i) { memcpy(target++, buffer++, sizeof(uint32_t)); }
	} else {
		for (uint16_t i = 0; i < length; ++i) { ct_reverse_memcpy(target++, buffer++, sizeof(uint32_t)); }
	}
...
uint16_t ct_packet_put_u64s(ct_packet_buf_t self, const uint64_t *buffer, uint16_t length, ct_endian_t endian) {
	...
	if (self->_total + sizeof(uint64_t) > self->_max) { return 0; }

	if ((length << 3) + self->_total > self->_max) { length = (self->_max - self->_total) >> 3; }

	uint64_t *target = (uint64_t *)(self->_buffer + self->_total);
	if (endian == CT_ENDIAN_SYSTEM) {
		for (uint16_t i = 0; i < length; ++i) { memcpy(target++, buffer++, sizeof(uint64_t)); }
	} else {
		for (uint16_t i = 0; i < length; ++i) { ct_reverse_memcpy(target++, buffer++, sizeof(uint64_t)); }
	}
