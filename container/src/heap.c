...
void ct_heap_down(ct_heap_buf_t self, size_t i) {
	...
	for (;;) {
		if (index_left < self->_size && CT_HEAP_COMPARE(self, index_left, i)) { i = index_left; }
		if (index_right < self->_size && CT_HEAP_COMPARE(self, index_right, i)) { i = index_right; }
		if (i == index_min) { break; }
		// 交换位置
...
void ct_heap_reorder(ct_heap_buf_t self) {
	// 自底向上构建堆
	for (size_t i = self->_size / 2; i > 0; i--) { ct_heap_down(self, i); }
}

void ct_heap_insert(ct_heap_buf_t self, ct_any_t data) {
	if (self->_size >= self->_max) { return; }
...
void ct_heap_remove(ct_heap_buf_t self) {
	// 判断堆是否为空
	if (ct_heap_isempty(self)) { return; }
...
ct_any_t ct_heap_take(ct_heap_buf_t self) {
	if (ct_heap_isempty(self)) { return ct_any_null; }
...
bool ct_heap_copy(ct_heap_buf_t self, ct_heap_t* target) {
	// 检查容量
	if (target->_max < self->_size) { return false; }
	// 拷贝所有元素
	for (size_t i = 0; i < self->_size; ++i) { target->_all[i] = self->_all[i]; }
