...
/* 合并两个堆 */
static ct_heap_node_t* _heap_meld(ct_heap_node_t* a, ct_heap_node_t* b, ct_heap_compare_cb cmp) {
	if (!a) { return b; }
	if (!b) { return a; }

	if (cmp(a, b) < 0) {
		/* a 优先级更高，b 成为 a 的第一个子节点 */
		b->right = a->child;
...
static ct_heap_node_t* _heap_merge_pairs_iter(ct_heap_node_t* first, ct_heap_compare_cb cmp) {
	...
	while (curr) {
		...
		if (b) {
			...
			ct_heap_node_t* paired                  = _heap_meld(a, b, cmp);
	...
	while (tree_list) {
		...
		root                 = _heap_meld(root, tree_list, cmp);
		tree_list            = next;
	}
...
void ct_heap_insert(ct_heap_t* heap, ct_heap_node_t* node) {
	...
	heap->_root                            = _heap_meld(heap->_root, node, heap->_cmp);
	++heap->_size;
}
...
ct_heap_node_t* ct_heap_pop(ct_heap_t* heap) {
	...
	heap->_root = _heap_merge_pairs_iter(children, heap->_cmp);
	--heap->_size;
...
void ct_heap_remove(ct_heap_t* heap, ct_heap_node_t* node) {
	...
	/* 合并剥离出来的子树到根节点 */
	ct_heap_node_t* remeld = _heap_merge_pairs_iter(children, heap->_cmp);
	heap->_root            = _heap_meld(heap->_root, remeld, heap->_cmp);
}
