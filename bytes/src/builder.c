...
static int ensure_capacity(ct_builder_t *self, size_t required) {
	...
	uint8_t *new_buffer = (uint8_t *)realloc(self->seg.bytes, new_cap);
	if (!new_buffer) {
		return -1;
	}
	self->seg.bytes = new_buffer;
...
void ct_builder_put_arr8(ct_builder_t *self, const uint8_t *v, size_t count) {
	...
	const size_t required = self->seg.pos + count;
	if (ensure_capacity(self, required) != 0) {
		return;
	}
	ct_seg_put_arr8(&self->seg, v, count);
}
...
void ct_builder_put_arr16(ct_builder_t *self, const uint16_t *v, size_t count) {
	...
	const size_t required = self->seg.pos + (count << 1);
	if (ensure_capacity(self, required) != 0) {
		return;
	}
	ct_seg_put_arr16(&self->seg, v, count);
}
...
void ct_builder_put_arr32(ct_builder_t *self, const uint32_t *v, size_t count) {
	...
	const size_t required = self->seg.pos + (count << 2);
	if (ensure_capacity(self, required) != 0) {
		return;
	}
	ct_seg_put_arr32(&self->seg, v, count);
}
...
void ct_builder_put_arr64(ct_builder_t *self, const uint64_t *v, size_t count) {
	...
	const size_t required = self->seg.pos + (count * 8);
	if (ensure_capacity(self, required) != 0) {
		return;
	}
	ct_seg_put_arr64(&self->seg, v, count);
}
