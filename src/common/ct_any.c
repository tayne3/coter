...
size_t ct_any_tostring(const ct_any_buf_t self, char *buf, size_t max)
{
	...
	switch (self->type) {
		...
		case CTAny_TypeInt8: return ct_snprintf(buf, max, "%" CT_PRIi8, self->d->i8);
		case CTAny_TypeInt16: return ct_snprintf(buf, max, "%" CT_PRIi16, self->d->i16);
		case CTAny_TypeInt32: return ct_snprintf(buf, max, "%" CT_PRIi32, self->d->i32);
		case CTAny_TypeInt64: return ct_snprintf(buf, max, "%" CT_PRIi64, self->d->i64);
		case CTAny_TypeUint: return ct_snprintf(buf, max, "%u", self->d->u);
		case CTAny_TypeUint8: return ct_snprintf(buf, max, "%" CT_PRIu8, self->d->u8);
		case CTAny_TypeUint16: return ct_snprintf(buf, max, "%" CT_PRIu16, self->d->u16);
		case CTAny_TypeUint32: return ct_snprintf(buf, max, "%" CT_PRIu32, self->d->u32);
		case CTAny_TypeUint64: return ct_snprintf(buf, max, "%" CT_PRIu64, self->d->u64);
...
void ct_any_set_uint(ct_any_buf_t self, uint_t value)
{
	assert(self);
...
uint_t ct_any_get_uint(const ct_any_buf_t self)
{
	assert(self);
	return self->d->u;
}
