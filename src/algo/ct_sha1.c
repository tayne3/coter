...
#define blk0_l(i) (block->l[i] = (rol(block->l[i], 24) & 0xFF00FF00) | (rol(block->l[i], 8) & 0x00FF00FF))
#define blk0_b(i) block->l[i]
#if defined(CTEndian_SystemLittle)
#define blk0(i) blk0_l(i)
#elif defined(CTEndian_SystemBig)
#define blk0(i) blk0_b(i)
#else
#define blk0(i) (CTEndian_System == CTEndian_Little ? blk0_l(i) : blk0_b(i))
#endif
#define blk(i)          \
	(block->l[i & 15] = \
...
void ct_sha1_update(ct_sha1_ctx_t *context, const unsigned char *data, uint32_t len) {
	...
	if ((j + len) > 63) {
		...
		for (; i + 63 < len; i += 64) {
			ct_sha1_transform(context->state, &data[i]);
		}
		j = 0;
	} else {
		i = 0;
...
void ct_sha1_string(char *hash_out, const char *str, uint32_t len) {
	...
	for (ii = 0; ii < len; ii += 1) {
		ct_sha1_update(&ctx, (const unsigned char *)str + ii, 1);
	}
