...
// 添加事件
static inline void ct_event_add(ct_event_buf_t self, ct_event_id_t id);
// 获取单个事件，不指定事件id
static inline bool ct_event_id_take(ct_event_buf_t self, ct_event_id_t *result);
// 获取单个事件，指定单个事件id
static inline bool ct_event_id_take_single(ct_event_buf_t self, ct_event_id_t id, ct_event_id_t *result);
// 获取单个事件，指定多个事件id
static inline bool ct_event_id_take_multiple(ct_event_buf_t self, ct_event_ids_t ids, ct_event_id_t *result);
// 参数设置
static inline void ct_event_id_arg_set(ct_event_buf_t self, ct_event_id_t id, ct_any_t arg);
// 参数获取
static inline ct_any_t ct_event_id_arg_get(ct_event_buf_t self, ct_event_id_t id);
// 参数获取
static inline void ct_event_id_arg_take(ct_event_buf_t self, ct_event_id_t id, ct_any_buf_t arg);
...
static inline bool ct_event_id_take(ct_event_buf_t self, ct_event_id_t *result)
{
	...
	for (size_t i = 1; i < CT_EVENT_ID_MAX; i++, mid <<= 1) {
		if (self->ids & mid) {
			if (result) {
				*result = i;
			}
