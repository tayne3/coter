...
int __ct_threadcache_basic(ct_threadcache_t *self, const char *fmt, ...) {
	...
	va_list args;
	va_start(args, fmt);
	result = vsnprintf(self->buffer, 1024, fmt, args);
	va_end(args);

	self->buffer[1023] = '\0';  // 确保字符串总是以 null 结尾
...
int __ct_threadcache_brief(ct_threadcache_t *self, const char *info, const char *fmt, ...) {
	...
	const int prefix_size = snprintf(self->buffer, 1024ULL, info, self->tm_str, self->tid_str);

	int     result;
	va_list args;
	va_start(args, fmt);
	result = vsnprintf(self->buffer + prefix_size, (size_t)1024 - prefix_size, fmt, args);
	va_end(args);
