...
// 定时器是否为空
static inline bool tr_isnull(ct_timer_t *self);
...
// 创建定时器
static inline ct_timer_id_t mgr_create_timer(ct_time64_t interval, bool isnow, bool is_loop,
...
void ct_timer_mgr_init(void) {
	...
	mgr->time_now  = gettimeofday_ms();
	mgr->time_tick = gettick_ms();
}

void ct_timer_mgr_schedule(ct_time64_t now, ct_time64_t tick) {
	mgr->time_now  = now;
	mgr->time_tick = tick;
...
ct_timer_id_t ct_timer_start(ct_time64_t interval, bool isnow, bool isloop, ct_timer_callback_t callback,
							 ct_any_t arg) {
	return mgr_create_timer(interval, isnow, isloop, callback, arg);
}

void ct_timer_stop(ct_timer_id_t id) {
	mgr_lock();
	ct_timer_t *self = tr_get(id);  // 根据id查找定时器
...
static inline bool tr_istrigger(ct_timer_t *self) {
	return self->trigger_new != 0 && self->trigger_new <= mgr->time_tick;
}
