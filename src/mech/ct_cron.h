...
// cron任务id类型
typedef uint64_t ct_cron_id_t;
// 无效的cron任务id
#define CT_CRON_ID_INVALID 0
// cron任务回调函数类型
typedef void (*ct_cron_callback_t)(ct_cron_id_t, const ct_any_buf_t);

/**
 * @brief cron任务管理初始化
 * @param now 当前时间 (s)
 */
CT_API void ct_cron_mgr_init(ct_time_t now);
 ...
 */
CT_API bool ct_cron_mgr_schedule(ct_time_t now);
 ...
 * @param id cron任务id
 */
CT_API void ct_cron_stop(ct_cron_id_t id);
