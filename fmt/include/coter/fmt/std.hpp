...
#ifndef COTER_FMT_FMT_STD_H_
#define COTER_FMT_FMT_STD_H_
...
auto write_escaped_alternative(OutputIt out, const T& v, FormatContext& ctx) -> OutputIt {
...
class is_variant_formattable {
    ...
    static auto check(std::index_sequence<Is...>)
        -> std::conjunction<is_formattable<std::variant_alternative_t<Is, Variant>, Char>...>;
...
#if FMT_USE_RTTI
inline auto normalize_libcxx_inline_namespaces(string_view demangled_name_view, char* begin) -> string_view {
    ...
    if (demangled_name_view.starts_with("std::")) {
        char* to = begin + 5;  // std::
        for (const char *from = to, *end = begin + demangled_name_view.size(); from < end;) {
...
auto normalize_msvc_abi_name(string_view abi_name_view, OutputIt out) -> OutputIt {
...
private:
    ...
    struct writer {
        ...
        FMT_CONSTEXPR auto operator()(OutputIt out) -> OutputIt {
            for (auto pos = N; pos > 0; --pos) out = detail::write<Char>(out, bs[pos - 1] ? Char('1') : Char('0'));
            return out;
        }
    };
...
struct formatter<
    ...
    auto format(const std::expected<T, E>& value, FormatContext& ctx) const -> decltype(ctx.out()) {
        ...
        if (value.has_value()) {
            ...
            if constexpr (!std::is_void<T>::value) out = detail::write_escaped_alternative<Char>(out, *value, ctx);
        } else {
...
struct formatter<
    Variant, Char,
    std::enable_if_t<std::conjunction_v<is_variant_like<Variant>, detail::is_variant_formattable<Variant, Char>>>> {
    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char* { return ctx.begin(); }
    ...
    auto format(const Variant& value, FormatContext& ctx) const -> decltype(ctx.out()) {
        ...
        FMT_TRY {
            std::visit([&](const auto& v) { out = detail::write_escaped_alternative<Char>(out, v, ctx); }, value);
        }
...
struct formatter<std::reference_wrapper<T>, Char,
                 ...
                 enable_if_t<is_formattable<remove_cvref_t<T>, Char>::value && !detail::has_format_as<T>::value &&
                             !detail::has_format_as_member<T>::value>> : formatter<remove_cvref_t<T>, Char> {
    template <typename FormatContext>
