...
#ifndef COTER_FMT_FMT_COLOR_H_
#define COTER_FMT_FMT_COLOR_H_
...
public:
    ...
    FMT_CONSTEXPR auto operator|=(text_style rhs) -> text_style& {
        if (((style_ + rhs.style_) & ((1ULL << 26) | (1ULL << 53))) != 0) report_error("can't OR a terminal color");
        style_ |= rhs.style_;
        return *this;
    }
...
struct ansi_color_escape {
    FMT_CONSTEXPR ansi_color_escape(color_type text_color, const char* esc) noexcept {
        ...
        for (int i = 0; i < 7; i++) { buffer[i] = static_cast<Char>(esc[i]); }
