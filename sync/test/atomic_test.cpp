...
static void test_atomic_ptr_compare_exchange(void) {
	...
	REQUIRE(success);
	REQUIRE(ct_atomic_ptr_load(&ptr) == &val2);
	REQUIRE(expected == &val1);  // Expected should not change on success

	// Failure case
	expected = &val3;  // Wrong expected value
	success  = ct_atomic_ptr_compare_exchange(&ptr, &expected, &val1);
	REQUIRE(!success);
	REQUIRE(ct_atomic_ptr_load(&ptr) == &val2);  // Should not change
	REQUIRE(expected == &val2);                  // Expected should be updated to current value
}
...
TEST_CASE("atomic_basic", "[atomic]") {
	SECTION("Ensures atomic flag can be set, tested, and cleared") { test_flag(); }
	SECTION("Ensures atomic load and store operate correctly") { test_load_store(); }
	SECTION("Ensures atomic add and sub work in a single thread") { test_add_sub(); }
}

TEST_CASE("atomic_edge", "[atomic]") {
	SECTION("Verifies that atomic operations return the previous value") { test_return_value(); }
	SECTION("Verifies that atomic operations handle integer overflow") { test_overflow(); }
	SECTION("Ensures explicit atomic types work correctly") { test_explicit_atomic_types(); }
}

TEST_CASE("atomic_ptr", "[atomic]") {
	SECTION("Basic pointer operations") { test_atomic_ptr_basic(); }
	SECTION("Pointer compare exchange") { test_atomic_ptr_compare_exchange(); }
	SECTION("CAS ABA behavior check") { test_cas_aba_behavior(); }
}
