...
struct rwlock_env {
	...
	rwlock_env() {
		ct_rwlock_init(&lock, NULL);
	}

	~rwlock_env() {
		ct_rwlock_destroy(&lock);
	}
};
...
static int reader_thread(void* arg) {
	...
	while (!ct_atomic_int_load(&env->release_readers)) {
		ct_msleep(1);
	}
...
static int try_reader_thread(void* arg) {
	...
	if (ct_atomic_int_load(&env->try_result) == 0) {
		ct_rwlock_rdunlock(&env->lock);
	}
	return 0;
}
...
TEST_CASE("rwlock_trywrlock_fails_while_reader_holds", "[sync][rwlock]") {
	rwlock_env  env;
	ct_thread_t thread;
...
TEST_CASE("rwlock_tryrdlock_fails_while_writer_holds", "[sync][rwlock]") {
	rwlock_env  env;
	ct_thread_t thread;
...
TEST_CASE("rwlock_allows_parallel_readers_and_blocks_writer", "[sync][rwlock]") {
	rwlock_env  env;
	...
	for (int i = 0; i < 40 && ct_atomic_int_load(&env.readers_inside) < 2; ++i) {
		ct_msleep(5);
	}
	...
	for (int i = 0; i < 20 && ct_atomic_int_load(&env.writer_acquired) == 0; ++i) {
		ct_msleep(5);
	}
