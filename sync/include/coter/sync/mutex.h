...
CT_INLINE int ct_mutex_init(ct_mutex_t* mutex) {
...
CT_INLINE int ct_mutex_destroy(ct_mutex_t* mutex) {
    if (!mutex) { return EINVAL; }
#ifdef CT_OS_WIN
    CT_UNUSED(mutex);
    return 0;
...
CT_INLINE int ct_mutex_lock(ct_mutex_t* mutex) {
    if (!mutex) { return EINVAL; }
#ifdef CT_OS_WIN
...
CT_INLINE int ct_mutex_unlock(ct_mutex_t* mutex) {
    if (!mutex) { return EINVAL; }
#ifdef CT_OS_WIN
...
CT_INLINE int ct_mutex_trylock(ct_mutex_t* mutex) {
    if (!mutex) { return EINVAL; }
#ifdef CT_OS_WIN
